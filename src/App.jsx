...
// â”€â”€â”€ GST Rates â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ PIN CODE â†’ CITY MAPPING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ CA MARKETPLACE WITH PIN CODE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800 }}>ðŸ‘¨â€ðŸ’¼ CA Marketplace</div>
          <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>{allCAs.length} Chartered Accountants across India â€” search by PIN code or city</div>
        </div>
        <button style={btn("success")} onClick={()=>setShowRegister(true)}>+ Register as CA</button>
      </div>

      {/* â”€â”€ PIN CODE SEARCH â”€â”€ */}
      <div style={{ ...card, marginBottom:16, padding:20, background:`linear-gradient(135deg, ${C.primaryLighter}, #fff)`, border:`1.5px solid ${C.primaryLight}40` }}>
        <div style={{ fontWeight:700, fontSize:14, marginBottom:12, color:C.primary }}>ðŸ“ Find CAs Near You â€” Search by PIN Code</div>
        <div style={{ display:"flex", gap:10, alignItems:"flex-end", flexWrap:"wrap" }}>
          ...
          <div>
            <button style={{ ...btn(), padding:"11px 24px", fontSize:14 }} onClick={searchByPin} disabled={pinLoading}>
              {pinLoading ? "ðŸ” Searchingâ€¦" : "ðŸ” Find CAs"}
            </button>
          </div>
          {pinResult && (
            <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 16px", background:C.successLight, borderRadius:8, border:`1px solid ${C.success}40` }}>
              <div>
                <div style={{ fontWeight:700, fontSize:13, color:C.success }}>âœ… {pinResult.area}, {pinResult.city}</div>
                <div style={{ fontSize:11, color:C.textMuted }}>{pinResult.state} â€” Showing CAs in {pinResult.city}</div>
              </div>
              <button onClick={clearPin} style={{ background:"none", border:"none", color:C.danger, cursor:"pointer", fontSize:18, marginLeft:4 }}>âœ•</button>
            </div>
          )}
          {pinError && <div style={{ color:C.danger, fontSize:13, fontWeight:600 }}>âš ï¸ {pinError}</div>}
        </div>
        <div style={{ fontSize:11, color:C.textMuted, marginTop:8 }}>
          ðŸ’¡ Try: <span style={{ cursor:"pointer", color:C.primary, fontWeight:600 }} onClick={()=>{setPinInput("452001"); setTimeout(searchByPin,100);}}>452001</span> (Indore) â€¢
          <span style={{ cursor:"pointer", color:C.primary, fontWeight:600, marginLeft:4 }} onClick={()=>{setPinInput("400001"); setTimeout(searchByPin,100);}}>400001</span> (Mumbai) â€¢
          <span style={{ cursor:"pointer", color:C.primary, fontWeight:600, marginLeft:4 }} onClick={()=>{setPinInput("110001"); setTimeout(searchByPin,100);}}>110001</span> (Delhi) â€¢
      ...
      <div style={{ marginBottom:14 }}>
        <input style={{ ...inp, padding:"11px 16px", fontSize:14, borderRadius:10 }} placeholder="ðŸ” Search by name, city, area, or specializationâ€¦" value={search} onChange={e=>setSearch(e.target.value)} />
      </div>
      ...
      <div style={{ ...card, marginBottom:20, padding:14 }}>
        <div style={{ display:"flex", gap:12, alignItems:"flex-end", flexWrap:"wrap" }}>
          ...
          <div style={{ flex:1, minWidth:140 }}>
            <div style={lbl}>Max â‚¹{filter.maxPrice.toLocaleString("en-IN")}/mo</div>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:11, color:C.textMuted }}>ðŸ“ {ca.area ? `${ca.area}, ` : ""}{ca.city}</div>
                    {ca.pincode && <div style={{ fontSize:10, color:C.primaryLight, fontWeight:600 }}>PIN: {ca.pincode}</div>}
                  </div>
                </div>
                {ca.verified ? <span style={badge(C.success)}>âœ“ Verified</span> : <span style={badge(C.textMuted)}>New</span>}
              </div>
              <div style={{ fontSize:12, color:C.textMuted, marginBottom:8 }}>ðŸŽ¯ {ca.specialization}</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:4, marginBottom:10 }}>
                {ca.languages.map(l=><span key={l} style={{ ...badge(C.primaryLight), fontSize:10 }}>{l}</span>)}
              </div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                <div>
                  <span style={{ fontSize:15, fontWeight:800, color:C.primary }}>â‚¹{ca.price.toLocaleString("en-IN")}</span>
                  <span style={{ fontSize:11, color:C.textMuted }}>/month</span>
                </div>
                <div style={{ fontSize:12 }}>
                  {ca.rating > 0 ? <>â­ {ca.rating} <span style={{ color:C.textMuted }}>({ca.reviews})</span></> : <span style={{ color:C.textMuted, fontSize:11 }}>No reviews yet</span>}
                </div>
              </div>
              <div style={{ fontSize:11, color:C.textMuted, marginBottom:12 }}>{ca.experience} yrs exp â€¢ {ca.experience > 10 ? "Senior CA" : ca.experience > 5 ? "Experienced" : "Junior CA"}</div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999, padding:20 }}>
          <div style={{ ...card, width:580, padding:32, maxHeight:"92vh", overflowY:"auto" }}>
            {registered ? (
              ...
              <>
                <div style={{ fontWeight:800, fontSize:18, color:C.primary, marginBottom:4 }}>ðŸ“‹ Register as CA on TaxSaathi</div>
                <div style={{ color:C.textMuted, fontSize:13, marginBottom:20 }}>Get discovered by clients in YOUR PIN code area â€” 10% commission on bookings only</div>

                {/* PIN CODE auto-fill */}
                <div style={{ background:C.primaryLighter, borderRadius:8, padding:"12px 16px", marginBottom:16 }}>
                  <div style={{ fontWeight:700, fontSize:13, color:C.primary, marginBottom:8 }}>ðŸ“ Enter Your Office PIN Code â€” We'll auto-fill your city!</div>
                  <div style={{ display:"flex", gap:10 }}>
                    ...
                    {newCA.city && <div style={{ display:"flex", alignItems:"center", padding:"0 14px", background:C.successLight, borderRadius:7, color:C.success, fontWeight:700, fontSize:13, whiteSpace:"nowrap" }}>âœ… {newCA.area}, {newCA.city}</div>}
                  </div>
                </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
                  ...
                  <div><div style={lbl}>Monthly Fee (â‚¹) *</div><input style={inp} type="number" placeholder="2000" value={newCA.price} onChange={e=>setNewCA({...newCA,price:e.target.value})} /></div>
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
                ...
                <div style={{ background:"#FFF9E6", borderRadius:8, padding:"12px 16px", fontSize:12, marginBottom:20, lineHeight:1.8 }}>
                  âœ… Profile visible immediately after submission<br/>
                  ðŸ” ICAI verification = "Verified" badge within 24 hrs<br/>
                  ðŸ“ Clients searching your PIN code will find you first<br/>
                  ðŸ’° No upfront cost â€” 10% commission on confirmed bookings only
                </div>
                <div style={{ display:"flex", gap:10 }}>
      ...
      {booked && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999 }}>
          <div style={{ ...card, width:500, padding:32 }}>
            <div style={{ fontWeight:800, fontSize:18, marginBottom:4 }}>ðŸ“… Book Consultation</div>
            <div style={{ display:"flex", gap:12, alignItems:"center", padding:"12px 16px", background:C.primaryLighter, borderRadius:8, marginBottom:20 }}>
              ...
              <div>
                ...
                <div style={{ fontSize:12, color:C.textMuted }}>ðŸ“ {booked.area ? `${booked.area}, ` : ""}{booked.city} (PIN: {booked.pincode||"â€”"}) â€¢ {booked.experience} yrs exp</div>
                <div style={{ fontSize:13, color:C.primary, fontWeight:600 }}>â‚¹{booked.price.toLocaleString("en-IN")}/month</div>
              </div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
            ...
            <div style={{ marginBottom:20 }}><div style={lbl}>Your requirement</div>
              <textarea style={{...inp,height:80,resize:"none"}} placeholder="e.g. GSTR-1 pending for 3 months, need urgent help with filingâ€¦" />
            </div>
            <div style={{ display:"flex", gap:10 }}>
              <button style={{ ...btn("success"), flex:1, justifyContent:"center" }} onClick={()=>{ alert(`âœ… Booking confirmed with ${booked.name}!\n\nðŸ“§ Email: ${booked.email}\nðŸ“± WhatsApp: ${booked.mobile}\nðŸ“ Area: ${booked.area||booked.city}\n\nYou will receive a confirmation shortly.`); setBooked(null); }}>
                âœ… Confirm Booking
              </button>
...
// â”€â”€â”€ useAuth â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function useAuth() {
...
// â”€â”€â”€ useData â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function AuthScreen({ onLogin, onSignup, onReset }) {
  ...
  const featureList = [
    { icon:"ðŸ“Š", title:"GST Reports & Filing", desc:"Auto-generate GSTR-1, GSTR-3B with one click" },
    { icon:"ðŸ§¾", title:"Invoice Generator", desc:"GST-compliant invoices in seconds" },
    { icon:"ðŸ¤–", title:"AI Tax Assistant", desc:"Ask anything in Hindi or English" },
    { icon:"ðŸ’°", title:"ITC Reconciliation", desc:"Maximize your Input Tax Credit" },
    { icon:"ðŸ“±", title:"E-Invoice & E-Way Bill", desc:"Direct GST portal integration" },
    { icon:"ðŸ‘¨â€ðŸ’¼", title:"CA Marketplace", desc:"Find verified CAs in your city" },
  ];

  const planList = [
    { name:"Free", price:"â‚¹0", desc:"Forever free", color:"#5D6D7E", features:["1 GSTIN","Unlimited Invoices","GST Reports","5 Clients"] },
    { name:"Starter", price:"â‚¹299/mo", desc:"For small business", color:"#2E86C1", features:["3 GSTINs","Excel Upload","50 Clients","AI Assistant"] },
    { name:"Pro", price:"â‚¹599/mo", desc:"Most Popular â­", color:"#1B4F72", popular:true, features:["10 GSTINs","E-Invoice","Bank Recon","Unlimited AI"] },
    { name:"Enterprise", price:"â‚¹799/mo", desc:"For large orgs", color:"#7D3C98", features:["Unlimited GSTINs","Dedicated CA","Custom Integration","SLA"] },
  ];
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI', Arial, sans-serif", background:"#F4F6F8", minHeight:"100vh", overflowY:"auto", overflowX:"hidden" }}>
      ...
      <div style={{ background:`linear-gradient(135deg, ${C.sidebar} 0%, ${C.primary} 100%)`, padding:"70px 5% 60px", textAlign:"center" }}>
        ...
        <p style={{ color:"rgba(255,255,255,0.75)", fontSize:16, maxWidth:560, margin:"0 auto 32px", lineHeight:1.7 }}>File GSTR-1, GSTR-3B, manage invoices, track ITC, and get AI-powered tax advice â€” all in one platform.</p>
      ...
      <div style={{ background:C.sidebar, padding:"28px 5%" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px, 1fr))", gap:20, maxWidth:900, margin:"0 auto", textAlign:"center" }}>
          {[["500+","Businesses Filing"],["99%","GST Accuracy"],["24/7","AI Support"],["â‚¹0","Setup Cost"]].map(([n,l])=>(
      ...
      <div style={{ background:"linear-gradient(135deg, #F39C12, #e67e22)", padding:"50px 5%", textAlign:"center" }}>
        <h2 style={{ fontSize:"clamp(1.4rem, 3vw, 2rem)", fontWeight:900, color:C.sidebar, marginBottom:12 }}>Aaj Hi Shuru Karein â€” Bilkul Free!</h2>
...
function InvoiceGenerator({ company, clients, saveInvoice }) {
  ...
  function printInvoice() {
    ...
    printWindow.document.write(`
      ...
      <div class="header">
        <div>
          <div class="company-name">ðŸ‡®ðŸ‡³ ${company?.company_name || "Your Company"}</div>
          <div>GSTIN: ${company?.gstin || "â€”"}</div>
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800 }}>ðŸ§¾ GST Invoice Generator</div>
          <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Create professional GST-compliant tax invoices</div>
        </div>
        <div style={{ display:"flex", gap:10 }}>
          <button style={btn("outline")} onClick={() => setPreview(!preview)}>{preview ? "âœï¸ Edit" : "ðŸ‘ Preview"}</button>
          <button style={btn("success")} onClick={handleSave} disabled={saving}>{saving ? "Savingâ€¦" : saved ? "âœ… Saved!" : "ðŸ’¾ Save Invoice"}</button>
          <button style={btn()} onClick={printInvoice}>ðŸ–¨ï¸ Print / PDF</button>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:20 }}>
        {/* Invoice Details */}
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>ðŸ“‹ Invoice Details</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
          ...
          {isInterstate && <div style={{ marginTop:10, padding:"8px 12px", background:"#FFF3CD", borderRadius:6, fontSize:12, color:C.warning, fontWeight:600 }}>âš¡ Interstate supply detected â€” IGST will be applied</div>}
        </div>

        {/* Customer Details */}
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>ðŸ‘¤ Bill To</div>
      ...
      <div style={{ ...card, marginBottom:20 }}>
        <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>ðŸ“¦ Items / Services</div>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", minWidth:900 }}>
            <thead>
              <tr>
                {["#","Description","HSN/SAC","Qty","Unit","Rate (â‚¹)","GST %","Taxable","Tax","Total",""].map(h=><th key={h} style={{...TH,padding:"8px 10px"}}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {inv.lines.map((l,i) => (
                <tr key={i}>
                  ...
                  <td style={TD}><button onClick={()=>{const lines=[...inv.lines];lines.splice(i,1);setInv({...inv,lines:lines.length?lines:[emptyLine()]});}} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:16}}>âœ•</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
// â”€â”€â”€ AI TAX ASSISTANT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function AIAssistant({ summary, company }) {
  const [messages, setMessages] = useState([
    { role:"assistant", content:`Namaste! ðŸ™ Main TaxSaathi AI hoon â€” aapka personal GST & Tax expert.\n\nMain inke baare mein madad kar sakta hoon:\nâ€¢ GST calculations & filing queries\nâ€¢ ITC claims & reconciliation\nâ€¢ Invoice & e-way bill guidance\nâ€¢ Income tax & TDS questions\nâ€¢ Compliance deadlines\n\nKuch bhi poochein â€” Hindi ya English mein! ðŸ˜Š` }
  ]);
  ...
  async function send(text) {
    ...
    setLoading(true);
    try {
      const context = `You are TaxSaathi AI â€” an expert Indian GST and tax assistant. 
      Current user company: ${company?.company_name || "Unknown"}, GSTIN: ${company?.gstin || "Unknown"}, State: ${company?.state || "Unknown"}.
      Financial summary: Total Sales â‚¹${summary?.totalSales?.toLocaleString("en-IN") || 0}, Total Purchases â‚¹${summary?.totalPurchase?.toLocaleString("en-IN") || 0}, Net GST Payable â‚¹${summary?.netTaxPayable?.toLocaleString("en-IN") || 0}, ITC Available â‚¹${summary?.itcAvailable?.toLocaleString("en-IN") || 0}.
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 120px)" }}>
      <div style={{ marginBottom:16 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ¤– AI Tax Assistant</div>
        <div style={{ fontSize:13, color:C.textMuted }}>Powered by Claude AI â€” Ask anything about GST, ITR, TDS in Hindi or English</div>
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", maxHeight:"100dvh", display:"flex", flexDirection:"column", gap:12, padding:"4px 0", marginBottom:16 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" }}>
            {m.role==="assistant" && (
              <div style={{ width:36, height:36, borderRadius:"50%", background:`linear-gradient(135deg, ${C.primary}, ${C.primaryLight})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0, marginRight:10, marginTop:2 }}>ðŸ¤–</div>
            )}
        ...
        {loading && (
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:36, height:36, borderRadius:"50%", background:`linear-gradient(135deg, ${C.primary}, ${C.primaryLight})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18 }}>ðŸ¤–</div>
      ...
      <div style={{ display:"flex", gap:10, background:C.white, padding:16, borderRadius:12, border:`1.5px solid ${C.border}`, boxShadow:"0 -2px 10px rgba(0,0,0,0.05)" }}>
        <input style={{ ...inp, border:"none", outline:"none", padding:"8px 4px", flex:1, fontSize:14 }} value={input} onChange={e=>setInput(e.target.value)} placeholder="GST ke baare mein kuch bhi poocheinâ€¦ (Hindi ya English)" onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()} />
        <button style={{ ...btn(), padding:"10px 20px" }} onClick={() => send()} disabled={loading || !input.trim()}>Send âž¤</button>
      </div>
    </div>
  );
}

// â”€â”€â”€ CA MARKETPLACE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// â”€â”€â”€ MULTI-COMPANY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function MultiCompany({ companies, activeCompany, setActiveCompany, addCompany, plan }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800 }}>ðŸ¢ My Companies</div>
          <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>{companies.length} of {limit} GSTINs used â€” <span style={{ color:C.primary, fontWeight:600 }}>{plan.toUpperCase()} Plan</span></div>
        </div>
        {companies.length < limit ? (
          <button style={btn()} onClick={()=>setShowAdd(!showAdd)}>+ Add Company</button>
        ) : (
          <button style={btn("accent")}>âš¡ Upgrade to Add More</button>
        )}
      </div>
      ...
      {showAdd && (
        <div style={{ ...card, marginBottom:20, border:`1.5px solid ${C.primary}40`, background:C.primaryLighter }}>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button style={btn()} onClick={save} disabled={saving}>{saving?"Savingâ€¦":"Save Company"}</button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
        {companies.map(co => (
          <div key={co.id} style={{ ...card, cursor:"pointer", border:`2px solid ${activeCompany?.id===co.id?C.primary:C.border}`, background:activeCompany?.id===co.id?C.primaryLighter:C.white, transition:"all 0.2s" }} onClick={()=>setActiveCompany(co)}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
              <div style={{ width:44, height:44, borderRadius:10, background:activeCompany?.id===co.id?C.primary:C.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22 }}>ðŸ¢</div>
              {activeCompany?.id===co.id && <span style={badge(C.primary)}>â— Active</span>}
            </div>
            ...
            <div style={{ fontSize:12, color:C.textMuted }}>ðŸ“ {co.state || "â€”"}</div>
...
// â”€â”€â”€ SUBSCRIPTION / BILLING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const COUPONS = {
  "TAXFREE2026":   { discount:100, type:"percent", plan:"pro",        desc:"100% off â€” Pro Plan FREE forever!" },
  "TAXSAATHI100":  { discount:100, type:"percent", plan:"enterprise", desc:"100% off â€” Enterprise Plan FREE!" },
  "CASPECIAL":     { discount:100, type:"percent", plan:"pro",        desc:"100% off â€” Pro Plan FREE for CAs!" },
  "LAUNCH50":      { discount:50,  type:"percent", plan:"starter",    desc:"50% off â€” Starter Plan" },
  "WELCOME299":    { discount:100, type:"percent", plan:"starter",    desc:"100% off â€” Starter Plan FREE!" },
  "ENTERPRISE99":  { discount:100, type:"percent", plan:"enterprise", desc:"100% off â€” Enterprise Plan FREE!" },
};
...
function Subscription({ plan, upgradePlan, user }) {
  ...
  function applyCoupon() {
    ...
    setTimeout(() => {
      ...
      if (c) {
        setCouponMsg({ text:`âœ… Coupon applied! ${c.desc}`, valid:true, planId:c.plan, code });
      } else {
        setCouponMsg({ text:"âŒ Invalid coupon code. Please check and try again.", valid:false });
      }
      setApplying(false);
    }, 800);
  }
  ...
  async function activateWithCoupon() {
    ...
    setSuccess(`ðŸŽ‰ ${PLANS.find(p=>p.id===planId)?.name} Plan activated for FREE using coupon ${couponMsg.code}!`);
...
async function handleUpgrade(planId) {
  ...
  // Send amount in RUPEES â€” create-order.js multiplies by 100 to get paise
  ...
  try {
    ...
    const options = {
      ...
      currency: 'INR',
      name: 'TaxSaathi',
      description: `${planName} Plan â€” Monthly Subscription`,
      ...
      handler: async function (response) {
        ...
        if (verifyData.success) {
          await upgradePlan(planId);
          setSuccess(`ðŸŽ‰ ${planName} Plan activated successfully! Welcome aboard!`);
        } else {
  ...
  return (
    <div>
      <div style={{ marginBottom:24 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ’³ Plans & Billing</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>
          Current plan: <strong style={{ color:C.primary }}>{PLANS.find(p=>p.id===plan)?.name || "Free"}</strong> â€” Cancel anytime, no hidden fees
        </div>
      </div>
      ...
      {/* â”€â”€ COUPON CODE BOX â”€â”€ */}
      <div style={{ ...card, marginBottom:24, background:`linear-gradient(135deg, #FFF9E6, #fff)`, border:`1.5px solid ${C.accent}40`, padding:20 }}>
        <div style={{ fontWeight:800, fontSize:15, marginBottom:4, color:C.accent }}>ðŸŽŸï¸ Have a Coupon Code?</div>
        ...
        <div style={{ fontSize:11, color:C.textMuted, marginTop:10 }}>
          ðŸ’¡ Enter your coupon code to unlock free or discounted access.
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:28 }}>
        {PLANS.map(p => (
          <div key={p.id} style={{ ...card, padding:24, border:`2px solid ${plan===p.id?p.color:p.popular?p.color+"44":C.border}`, position:"relative", background:plan===p.id?p.color+"0a":C.white }}>
            ...
            {plan===p.id && <div style={{ position:"absolute", top:-12, right:16, background:C.success, color:C.white, fontSize:11, fontWeight:700, padding:"3px 12px", borderRadius:20 }}>âœ“ CURRENT</div>}
            <div style={{ fontWeight:800, fontSize:16, color:p.color, marginBottom:8 }}>{p.name}</div>
            <div style={{ marginBottom:16 }}>
              <span style={{ fontSize:32, fontWeight:900, color:p.color }}>â‚¹{p.price}</span>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
              {p.features.map(f => (
                <div key={f} style={{ display:"flex", gap:8, fontSize:12, alignItems:"flex-start" }}>
                  <span style={{ color:C.success, flexShrink:0, marginTop:1 }}>âœ“</span>
            ...
            {plan === p.id ? (
              ...
              <button style={{ ...btn(), width:"100%", justifyContent:"center", fontSize:12, background:p.color }} onClick={()=>handleUpgrade(p.id)} disabled={loading===p.id}>
                {loading===p.id ? "Processingâ€¦" : `Upgrade â€” â‚¹${p.price}/mo`}
              </button>
            )}
          </div>
        ))}
      ...
      {/* Feature Comparison */}
      <div style={card}>
        <div style={{ fontWeight:700, fontSize:16, marginBottom:16 }}>ðŸ“Š Why TaxSaathi beats Zoho Books</div>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {[
                ["Unlimited Invoices","âœ…","âœ…","âŒ (capped at plan limit)"],
                ["AI Tax Assistant","âŒ","âœ… Unlimited","âŒ Not available"],
                ["CA Marketplace","âŒ","âœ…","âŒ Not available"],
                ["Hindi Language UI","ðŸ”œ","âœ…","âŒ English only"],
                ["Tally/Busy Import","âŒ","âœ…","âŒ Limited"],
                ["WhatsApp Reminders","âŒ","âœ…","âŒ Email only"],
                ["GSTR-2B Auto-Reconciliation","âœ…","âœ…","ðŸ’° Paid add-on"],
                ["Offline Mode","âŒ","âœ…","âŒ"],
                ["Price","â‚¹0","â‚¹799/mo","â‚¹1,799/mo"],
              ].map(([feat,...vals])=>(
                <tr key={feat}>
                  <td style={{...TD,fontWeight:600}}>{feat}</td>
                  {vals.map((v,i)=><td key={i} style={{...TD,textAlign:"center",color:v.startsWith("âœ…")?C.success:v.startsWith("âŒ")?C.danger:v.startsWith("ðŸ’°")?C.warning:C.text}}>{v}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
...
function Dashboard({ summary, profile, plan }) {
  ...
  const stats = [
    { label:"Total Sales", value:fmt(totalSales), color:C.primary, icon:"ðŸ“ˆ" },
    { label:"Total Purchases", value:fmt(totalPurchase), color:C.primaryLight, icon:"ðŸ“‰" },
    { label:"GST Collected", value:fmt(cgstTotal+sgstTotal+igstTotal), color:C.accent, icon:"ðŸ§¾" },
    { label:"ITC Available", value:fmt(itcAvailable), color:C.success, icon:"ðŸ’°" },
    { label:"Net GST Payable", value:fmt(netTaxPayable), color:C.danger, icon:"âš ï¸" },
  ];
  return (
    <div>
      <div style={{ marginBottom:24, display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <div>
          <div style={{ fontSize:22, fontWeight:800 }}>{new Date().getHours()<12?"Good Morning":new Date().getHours()<17?"Good Afternoon":"Good Evening"}, {profile?.name?.split(" ")[0] || auth?.user?.email?.split("@")[0] || "User"} ðŸ‘‹</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:16 }}>âš¡ Quick Actions</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            {[
              {label:"Create Invoice",icon:"ðŸ§¾",color:C.primary},
              {label:"Upload Sales",icon:"ðŸ“¤",color:C.primaryLight},
              {label:"Ask AI Assistant",icon:"ðŸ¤–",color:C.purple},
              {label:"Find CA",icon:"ðŸ‘¨â€ðŸ’¼",color:C.success},
            ].map((a,i) => (
        ...
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:14 }}>â° Upcoming Deadlines</div>
          {(() => {
      ...
      <div style={{ ...card, marginTop:20 }}>
        <div style={{ fontWeight:700, fontSize:15, marginBottom:16 }}>ðŸ“Š Monthly GST Overview â€” FY {new Date().getMonth() >= 3 ? new Date().getFullYear() : new Date().getFullYear()-1}-{(new Date().getMonth() >= 3 ? new Date().getFullYear()+1 : new Date().getFullYear()).toString().slice(2)}</div>
...
// â”€â”€â”€ WHATSAPP NOTIFICATIONS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function WhatsAppNotifications({ clients, company }) {
  ...
  const TEMPLATES = {
    hindi: (dl, co) => `ðŸ‡®ðŸ‡³ *TaxSaathi Alert*\n\nNamaste! ðŸ™\n\n*${co?.company_name || "Aapki Company"}* ki taraf se yaad dilana chahte hain:\n\nâš ï¸ *${dl?.task}* ki deadline nazar aa rahi hai\nðŸ“… Due Date: *${dl?.date}*\nðŸ“‹ Period: ${dl?.period}\n\n${dl?.days <= 3 ? "ðŸš¨ URGENT: Sirf " + dl?.days + " din baaki hain!" : dl?.days <= 7 ? "â° " + dl?.days + " din baaki hain" : "âœ… " + dl?.days + " din baaki hain"}\n\nJaldi filing karein aur penalty se bachein! ðŸ’ª\n\n_TaxSaathi â€” GST Compliance Platform_`,
    english: (dl, co) => `ðŸ‡®ðŸ‡³ *TaxSaathi Reminder*\n\nDear Valued Client,\n\nThis is a reminder from *${co?.company_name || "Your CA"}*:\n\nâš ï¸ *${dl?.task}* deadline is approaching\nðŸ“… Due Date: *${dl?.date}*\nðŸ“‹ Period: ${dl?.period}\n\n${dl?.days <= 3 ? "ðŸš¨ URGENT: Only " + dl?.days + " days remaining!" : "ðŸ“Œ " + dl?.days + " days remaining"}\n\nPlease complete your filing to avoid penalties.\n\n_Powered by TaxSaathi_`,
    custom: () => customMsg,
  };
  ...
  function sendWhatsApp(mobile, msg) {
    ...
    if (!opened) {
      // Popup blocked â€” fallback to direct link
  ...
  async function sendBulk() {
    ...
    if (clientsWithMobile.length === 1) {
      ...
      alert(`âœ… WhatsApp opened for ${clientsWithMobile[0].company_name}!`);
      return;
    }

    // Multiple clients â€” open one by one with user confirmation
    setSending(true);
    ...
    setSending(false);
    alert(`âœ… Done! Sent to ${sent.length + clientsWithMobile.length} clients.`);
  }

  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ’¬ WhatsApp Notifications</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Send GST deadline reminders to clients via WhatsApp â€” one click!</div>
      </div>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:20, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden", width:"fit-content" }}>
        {[["send","ðŸ“¤ Send Notifications"],["history","ðŸ“‹ Sent History"],["templates","âœï¸ Templates"]].map(([id,label]) => (
      ...
      {tab === "send" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>Step 1 â€” Select Deadline to Remind</div>
            ...
            {/* Step 2: Template */}
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:12, color:C.primary }}>Step 2 â€” Message Template</div>
              <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                {[["hindi","à¤¹à¤¿à¤‚à¤¦à¥€"],["english","English"],["custom","Custom"]].map(([id,label]) => (
              ...
              {template === "custom" ? (
                <textarea style={{ ...inp, height:120, resize:"none", fontSize:12 }} value={customMsg} onChange={e => setCustomMsg(e.target.value)} placeholder="Type your custom WhatsApp message hereâ€¦" />
              ) : (
            ...
            {/* Schedule */}
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:12, color:C.primary }}>Step 3 â€” Schedule (Optional)</div>
              ...
              <div style={{ fontSize:11, color:C.textMuted, marginTop:8 }}>ðŸ’¡ Leave empty to send immediately. Scheduled messages open WhatsApp at the set time.</div>
            </div>
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={card}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                <div style={{ fontWeight:700, fontSize:14, color:C.primary }}>Step 4 â€” Select Clients</div>
                    ...
                    return (
                      <div key={c.id} onClick={() => setSelected(s => ({ ...s, clients: isSelected ? s.clients.filter(x => x.id !== c.id) : [...s.clients, c] }))}
                        ...
                        <div style={{ width:20, height:20, borderRadius:4, border:`2px solid ${isSelected?C.success:C.border}`, background:isSelected?C.success:"transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                          {isSelected && <span style={{ color:C.white, fontSize:12 }}>âœ“</span>}
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ fontWeight:600, fontSize:13 }}>{c.company_name}</div>
                          <div style={{ fontSize:11, color:C.textMuted }}>ðŸ“± {c.mobile || "No mobile"} â€¢ {c.state}</div>
                        </div>
            ...
            <div style={card}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:14 }}>
                <div><div style={{ fontWeight:700, fontSize:15 }}>Ready to Send</div>
                  <div style={{ fontSize:12, color:C.textMuted, marginTop:2 }}>{selected.clients.length} clients selected â€¢ {selected.clients.filter(c=>c.mobile).length} have WhatsApp</div>
                </div>
              ...
              {selected.clients.filter(c=>!c.mobile).length > 0 && (
                <div style={{ background:"#FFF9E6", borderRadius:7, padding:"8px 12px", fontSize:12, color:C.warning, marginBottom:12 }}>
                  âš ï¸ {selected.clients.filter(c=>!c.mobile).length} clients have no mobile number â€” they will be skipped
                </div>
              )}
              <button style={{ ...btn("success"), width:"100%", justifyContent:"center", padding:"13px", fontSize:15 }}
                ...
                {sending ? "â³ Sendingâ€¦" : `ðŸ“² Send WhatsApp to ${selected.clients.filter(c=>c.mobile).length} Clients`}
              </button>
              <div style={{ fontSize:11, color:C.textMuted, textAlign:"center", marginTop:8 }}>Opens WhatsApp Web for each client â€” confirm send in WhatsApp</div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === "history" && (
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:14 }}>ðŸ“‹ Sent Notifications</div>
          {sent.length === 0 ? (
            ...
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              ...
              <tbody>{sent.map((s,i) => (
                <tr key={i}>
                  ...
                  <td style={TD}><span style={badge(C.success)}>âœ“ Sent</span></td>
                </tr>
              ))}</tbody>
            </table>
          )}
      ...
      {tab === "templates" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          {[
            { name:"Hindi Reminder", lang:"à¤¹à¤¿à¤‚à¤¦à¥€", desc:"Friendly Hindi message for local clients" },
            ...
            <div key={i} style={card}>
              ...
              <div style={{ background:C.bg, borderRadius:7, padding:10, fontSize:11, color:C.text, lineHeight:1.6 }}>
                {i===0 && `ðŸ‡®ðŸ‡³ TaxSaathi Alert\n\nNamaste! GSTR-1 filing ki deadline nazar aa rahi hai...\n[Client ka naam], Due: 11 Apr 2025`}
                {i===1 && `ðŸ‡®ðŸ‡³ TaxSaathi Reminder\n\nDear Client, GSTR-3B deadline is approaching.\nDue Date: 20 Apr 2025. Please file to avoid penalty.`}
                {i===2 && `ðŸš¨ URGENT: Sirf 3 din baaki hain!\n\nGSTR-1 filing abhi karein warna â‚¹10,000 penalty lagegi!\nCall karo: [CA mobile]`}
                {i===3 && `âš ï¸ Deadline Miss Ho Gayi!\n\nGSTR-1 ki due date kal thi. Late filing fee lagegi.\nAbhi contact karein: [CA name]`}
              </div>
              <button style={{ ...btn("outline"), marginTop:12, width:"100%", justifyContent:"center", fontSize:12 }}>âœï¸ Edit Template</button>
            </div>
          ))}
        </div>
      )}
...
// â”€â”€â”€ CA ENROLLMENT / PAYMENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function CAEnrollment() {
  const COURSES = [
    { id:"gst_basics",    title:"GST Fundamentals",         duration:"4 hrs",  lessons:12, level:"Beginner",      icon:"ðŸ“˜", color:C.primaryLight, topics:["GST Structure & Rates","GSTIN Registration","Input Tax Credit","Filing GSTR-1 & 3B"] },
    { id:"gst_advanced",  title:"Advanced GST Filing",      duration:"6 hrs",  lessons:18, level:"Intermediate",  icon:"ðŸ“—", color:C.success,      topics:["GSTR-9 Annual Return","E-Invoice & E-Way Bill","GST Audit","Composition Scheme"] },
    { id:"itr_filing",    title:"ITR Filing Masterclass",   duration:"5 hrs",  lessons:15, level:"Intermediate",  icon:"ðŸ“™", color:C.accent,       topics:["ITR-1 to ITR-4","Capital Gains","TDS Reconciliation","Refund Claims"] },
    { id:"tds_tcs",       title:"TDS & TCS Compliance",     duration:"3 hrs",  lessons:10, level:"Beginner",      icon:"ðŸ“’", color:C.purple,       topics:["TDS Rates & Sections","Form 16 & 26AS","TDS Returns","Late Filing Penalties"] },
    { id:"startup",       title:"Startup & MSME Compliance",duration:"4 hrs",  lessons:14, level:"Advanced",      icon:"ðŸš€", color:C.danger,       topics:["Company Incorporation","ROC Filings","MSME Registration","Startup Tax Benefits"] },
    { id:"ca_practice",   title:"Building Your CA Practice", duration:"3 hrs",  lessons:9,  level:"Advanced",     icon:"ðŸ’¼", color:C.primary,      topics:["Client Acquisition","Fee Structuring","Digital CA Tools","Growing on TaxSaathi"] },
  ];
  ...
  if (done) return (
    <div style={{ ...card, textAlign:"center", maxWidth:560, margin:"40px auto", padding:48 }}>
      <div style={{ fontSize:64, marginBottom:8 }}>ðŸŽ‰</div>
      ...
      <div style={{ color:C.textMuted, marginTop:10, lineHeight:1.9, fontSize:14 }}>
        ...
        Your profile is under review â€” goes live within <strong>2 hours</strong>.<br/>
      ...
      <div style={{ ...card, margin:"24px 0", background:C.primaryLighter, textAlign:"left", padding:20 }}>
        <div style={{ fontWeight:700, marginBottom:10, fontSize:14 }}>ðŸ“‹ What happens next:</div>
        {[
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:20, display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:12 }}>
        <div>
          <div style={{ fontSize:22, fontWeight:900 }}>ðŸ… CA Enrollment â€” 100% Free</div>
          <div style={{ fontSize:13, color:C.success, fontWeight:700, marginTop:4 }}>âœ… No fees â€¢ No commission â€¢ Join India's fastest growing CA network</div>
        </div>
        <div style={{ display:"flex", background:C.bg, borderRadius:8, border:`1px solid ${C.border}`, overflow:"hidden" }}>
          {[["register","ðŸ‘¨â€ðŸ’¼ CA Registration"],["courses","ðŸ“š Free Courses"]].map(([id,label]) => (
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:24 }}>
        {[
          { label:"CAs Enrolled", value:"35+",   icon:"ðŸ‘¨â€ðŸ’¼", color:C.primary },
          { label:"Cities Covered",value:"20+",  icon:"ðŸ“",  color:C.success },
          { label:"Free Courses",  value:"6",    icon:"ðŸ“š",  color:C.accent },
          { label:"Avg Leads/mo",  value:"8-15", icon:"ðŸ“ˆ",  color:C.purple },
        ].map((s,i) => (
        ...
        <div>
          ...
          <div style={{ display:"flex", gap:0, marginBottom:24, background:C.bg, borderRadius:10, overflow:"hidden", border:`1px solid ${C.border}` }}>
            {[["1","Basic Info"],["2","Professional Details"],["3","Course Selection"]].map(([n,label],i) => (
              <div key={n} onClick={()=>step>i+1&&setStep(i+1)} style={{ flex:1, padding:"12px 16px", textAlign:"center", background:step===i+1?C.primary:step>i+1?C.primaryLighter:"transparent", color:step===i+1?C.white:step>i+1?C.primary:C.textMuted, fontWeight:step===i+1?700:400, fontSize:13, cursor:step>i+1?"pointer":"default", borderRight:i<2?`1px solid ${C.border}`:"none", transition:"all 0.2s" }}>
                <span style={{ fontWeight:800 }}>{step>i+1?"âœ“":n}.</span> {label}
              </div>
            ))}
          </div>
          ...
          {step === 1 && (
            <div style={{ maxWidth:680, margin:"0 auto" }}>
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:15, marginBottom:18, color:C.primary }}>ðŸ‘¤ Basic Information</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
          ...
          {/* STEP 2 â€” Professional Details */}
          {step === 2 && (
            <div style={{ maxWidth:680, margin:"0 auto" }}>
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:15, marginBottom:18, color:C.primary }}>ðŸ’¼ Professional Details</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
                  ...
                  <div><div style={lbl}>Consultation Fee (â‚¹/month)</div><input style={inp} type="number" placeholder="1500" value={form.consultFee} onChange={e=>setForm({...form,consultFee:e.target.value})} /></div>
                </div>
                <div style={{ marginBottom:14 }}>
                ...
                <div style={{ display:"flex", gap:10 }}>
                  <button style={{ ...btn("outline") }} onClick={()=>setStep(1)}>â† Back</button>
                  <button style={{ ...btn(), flex:1, justifyContent:"center", padding:"11px" }} onClick={()=>setStep(3)}>
                    Next: Select Free Courses â†’
                  </button>
                </div>
              </div>
            </div>
          ...
          {/* STEP 3 â€” Course Selection */}
          {step === 3 && (
            <div style={{ maxWidth:780, margin:"0 auto" }}>
              <div style={{ ...card, marginBottom:16, background:C.primaryLighter, border:`1.5px solid ${C.primaryLight}40`, padding:16 }}>
                <div style={{ fontWeight:700, color:C.primary, marginBottom:4 }}>ðŸŽ Select Free Courses to Enroll</div>
                ...
                {COURSES.map(c => {
                  ...
                  return (
                    <div key={c.id} onClick={()=>toggleCourse(c.id)} style={{ ...card, cursor:"pointer", border:`2px solid ${selected?c.color:C.border}`, background:selected?c.color+"0d":C.white, transition:"all 0.2s", position:"relative" }}>
                      {selected && <div style={{ position:"absolute", top:12, right:12, width:22, height:22, borderRadius:"50%", background:c.color, color:C.white, fontSize:12, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center" }}>âœ“</div>}
                      <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <div style={{ display:"flex", gap:10, marginBottom:8 }}>
                            <span style={{ fontSize:11, color:C.textMuted }}>â± {c.duration}</span>
                            <span style={{ fontSize:11, color:C.textMuted }}>ðŸ“– {c.lessons} lessons</span>
              ...
              <div style={{ ...card, background:C.primaryLighter, marginBottom:16, padding:16 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    <div style={{ fontWeight:700, fontSize:14 }}>ðŸ… CA Registration â€” FREE</div>
                  ...
                  <div style={{ fontSize:24, fontWeight:900, color:C.success }}>â‚¹0</div>
                </div>
              </div>
              <div style={{ display:"flex", gap:10 }}>
                <button style={{ ...btn("outline") }} onClick={()=>setStep(2)}>â† Back</button>
                <button style={{ ...btn("success"), flex:1, justifyContent:"center", padding:"13px", fontSize:15 }} onClick={handleSubmit} disabled={submitting}>
                  {submitting ? "â³ Submittingâ€¦" : "ðŸš€ Complete Free Enrollment â†’"}
                </button>
              </div>
            </div>
          )}
      ...
      {tab === "courses" && (
        <div>
          <div style={{ ...card, background:`linear-gradient(135deg, ${C.primaryLighter}, #fff)`, border:`1.5px solid ${C.primaryLight}40`, marginBottom:20, padding:20 }}>
            <div style={{ fontWeight:800, fontSize:16, color:C.primary, marginBottom:4 }}>ðŸ“š Free CA Learning Centre</div>
              ...
              return (
                <div key={c.id} style={{ ...card, border:`1.5px solid ${C.border}`, transition:"all 0.2s" }}>
                  ...
                  <div style={{ display:"flex", gap:14, marginBottom:12 }}>
                    <span style={{ fontSize:12, color:C.textMuted }}>â± {c.duration}</span>
                    <span style={{ fontSize:12, color:C.textMuted }}>ðŸ“– {c.lessons} lessons</span>
                  </div>
                  <div style={{ marginBottom:14 }}>
                    {c.topics.map(t=>(
                      <div key={t} style={{ fontSize:12, color:C.textMuted, padding:"3px 0", display:"flex", gap:6 }}>
                        <span style={{ color:c.color }}>â€º</span>{t}
                      </div>
                    ))}
                  </div>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                    <span style={{ fontSize:18, fontWeight:900, color:C.success }}>FREE</span>
                    {enrolled && <span style={{ ...badge(C.success), fontSize:10 }}>âœ“ Enrolled</span>}
                  </div>
                  <button onClick={()=>{ enrollCourse(c.id); setActiveCourse(c); }} style={{ ...btn(enrolled?"outline":"success"), width:"100%", justifyContent:"center", fontSize:13 }}>
                    {enrolled ? "â–¶ Continue Learning" : "ðŸŽ“ Enroll & Start"}
                  </button>
                </div>
              );
            })}
...
// â”€â”€â”€ AI COURSE PLAYER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function AICoursePlayer({ course, onBack }) {
  ...
  function renderContent(text) {
    return text.split("\n").map((line, i) => {
      ...
      if (line.startsWith("* ") || line.startsWith("- ")) return <div key={i} style={{ display:"flex", gap:8, fontSize:13, lineHeight:1.7, marginBottom:4 }}><span style={{ color:C.primary, flexShrink:0 }}>â€º</span><span>{line.slice(2).replace(/\*\*(.*?)\*\*/g, (_,m)=>m)}</span></div>;
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20 }}>
        <button onClick={onBack} style={{ ...btn("outline"), padding:"8px 14px", fontSize:13 }}>â† Back</button>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:18, fontWeight:900 }}>{course.icon} {course.title}</div>
          <div style={{ fontSize:12, color:C.textMuted, marginTop:2 }}>{course.duration} â€¢ {lessons.length} lessons â€¢ {course.level}</div>
        </div>
        ...
        <div style={card}>
            ...
            return (
              <div key={i} onClick={() => loadLesson(lesson)} style={{ display:"flex", gap:10, alignItems:"flex-start", padding:"10px 12px", borderRadius:8, marginBottom:6, cursor:"pointer", background:active?C.primary+"12":done?"#F0FFF4":"transparent", border:`1px solid ${active?C.primary:done?C.success+"44":C.border}`, transition:"all 0.15s" }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:10, color:C.textMuted, marginTop:2 }}>â± {lesson.duration}</div>
                </div>
              </div>
            );
          })}
          {completed.length === lessons.length && (
            <div style={{ marginTop:12, padding:12, background:C.successLight, borderRadius:8, textAlign:"center", border:`1px solid ${C.success}40` }}>
              <div style={{ fontSize:20, marginBottom:4 }}>ðŸŽ“</div>
          ...
          {!selectedLesson ? (
            <div style={{ ...card, textAlign:"center", padding:48 }}>
              ...
              <div style={{ color:C.textMuted, fontSize:13, marginBottom:20, lineHeight:1.7 }}>
                This course is powered by AI â€” each lesson is generated fresh and tailored to Indian tax practice. Click any lesson on the left to begin!
              </div>
            ...
            <div>
              <div style={{ ...card, marginBottom:16 }}>
                ...
                {loading ? (
                  <div style={{ textAlign:"center", padding:"32px 0" }}>
                    ...
                    <div style={{ fontWeight:600, marginBottom:6 }}>AI is generating your lessonâ€¦</div>
              ...
              {!loading && (
                <div style={{ display:"flex", gap:10, marginBottom:16 }}>
                  {selectedLesson.id > 0 && (
                    <button style={{ ...btn("outline"), fontSize:12 }} onClick={() => loadLesson(lessons[selectedLesson.id - 1])}>â† Previous</button>
                  )}
                  {selectedLesson.id < lessons.length - 1 && (
                    <button style={{ ...btn("success"), fontSize:12, marginLeft:"auto" }} onClick={() => loadLesson(lessons[selectedLesson.id + 1])}>Next Lesson â†’</button>
                  )}
                </div>
              )}
              ...
              {!loading && (
                <div style={card}>
                  <div style={{ fontWeight:700, fontSize:14, marginBottom:12, color:C.primary }}>ðŸ’¬ Ask AI About This Lesson</div>
                  {qaHistory.map((m, i) => (
                  ...
                  {qaLoading && <div style={{ fontSize:13, color:C.textMuted, padding:"8px 0" }}>â³ AI is thinkingâ€¦</div>}
                  <div style={{ display:"flex", gap:10, marginTop:12 }}>
                    <input style={{ ...inp, flex:1, fontSize:13 }} placeholder="Ask a question about this lessonâ€¦" value={question} onChange={e=>setQuestion(e.target.value)} onKeyDown={e=>e.key==="Enter"&&askQuestion()} />
...
// â”€â”€â”€ E-INVOICE GENERATOR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function EInvoice({ company, sales }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ§¾ E-Invoice Generation</div>
      ...
      <div style={{ ...card, marginBottom:20, background:"#EBF5FB", border:`1px solid ${C.primaryLight}40` }}>
        <div style={{ display:"flex", gap:16, alignItems:"flex-start" }}>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:6 }}>E-Invoice is mandatory for businesses with turnover above â‚¹5 Crore</div>
            <div style={{ fontSize:13, color:C.textMuted, lineHeight:1.7 }}>
              â€¢ IRN is a unique 64-character hash generated by the GSTN portal<br/>
              â€¢ QR code must be printed on every B2B invoice<br/>
              â€¢ E-invoice must be generated before or at the time of issuing invoice
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
                ...
                <div key={inv.id} style={{ padding:"12px 14px", borderRadius:8, border:`1.5px solid ${selected===inv.id?C.primary:C.border}`, background:selected===inv.id?C.primaryLighter:C.white, cursor:"pointer" }} onClick={() => setSelected(inv.id)}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div>
                      ...
                      <div style={{ fontSize:11, color:C.textMuted }}>{inv.customer_name} â€¢ {inv.invoice_date}</div>
                      <div style={{ fontSize:12, color:C.primary, fontWeight:600 }}>{fmt(inv.total_value)}</div>
                    </div>
                    <div>
                      {irn[inv.id] ? (
                        <span style={badge(C.success)}>âœ“ IRN Generated</span>
                      ) : (
                        <button style={{ ...btn(), fontSize:11, padding:"6px 12px" }} onClick={e => { e.stopPropagation(); handleGenerate(inv); }} disabled={generating && selected===inv.id}>
                          {generating && selected===inv.id ? "â³ Generatingâ€¦" : "Generate IRN"}
                        </button>
                      )}
                    </div>
                  </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          {selected && irn[selected] ? (
            <>
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.success }}>âœ… IRN Generated Successfully!</div>
                <div style={{ marginBottom:12 }}>
                ...
                <div style={{ display:"flex", gap:10 }}>
                  <button style={{ ...btn(), fontSize:12 }} onClick={() => navigator.clipboard.writeText(irn[selected])}>ðŸ“‹ Copy IRN</button>
                  <button style={{ ...btn("outline"), fontSize:12 }} onClick={() => window.print()}>ðŸ–¨ï¸ Print with QR</button>
                </div>
              </div>
              {qr[selected] && (
                <div style={{ ...card, textAlign:"center" }}>
                  <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>ðŸ“± QR Code for Invoice</div>
                  ...
                  <a href={qr[selected]} download="qr_code.png">
                    <button style={{ ...btn("outline"), marginTop:10, fontSize:12, width:"100%", justifyContent:"center" }}>â¬‡ï¸ Download QR Code</button>
                  </a>
                </div>
              )}
            </>
          ...
          {/* E-Invoice Steps */}
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:12 }}>ðŸ“‹ E-Invoice Process</div>
            {[
              ...
              <div key={s.step} style={{ display:"flex", gap:12, padding:"8px 0", borderBottom:`1px solid ${C.border}` }}>
                <div style={{ width:28, height:28, borderRadius:"50%", background:s.done?C.success:C.bg, border:`2px solid ${s.done?C.success:C.border}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                  <span style={{ fontSize:12, color:s.done?C.white:C.textMuted, fontWeight:700 }}>{s.done?"âœ“":s.step}</span>
                </div>
                <div>
...
// â”€â”€â”€ E-WAY BILL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function EWayBill({ company, sales }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸšš E-Way Bill Generation</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Generate e-way bill for goods movement above â‚¹50,000</div>
      </div>
      ...
      {generated ? (
        <div>
          <div style={{ ...card, background:C.successLight, border:`1.5px solid ${C.success}40`, marginBottom:20 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              <div>
                <div style={{ fontWeight:800, fontSize:18, color:C.success }}>âœ… E-Way Bill Generated!</div>
                ...
                <div style={{ fontSize:13, color:C.textMuted, marginTop:6 }}>Generated: {generated.ewb_date} â€¢ Valid till: {generated.valid_till}</div>
              </div>
              <div style={{ display:"flex", gap:10 }}>
                <button style={{ ...btn(), fontSize:12 }} onClick={() => window.print()}>ðŸ–¨ï¸ Print</button>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            {[
              ...
              ["Transport Mode", generated.transport_mode], ["Vehicle No", generated.vehicle_no || "â€”"],
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, color:C.primary, marginBottom:14 }}>ðŸ“‹ Bill Details</div>
              <div style={{ marginBottom:12 }}>
                ...
                <select style={inp} value={form.invoice_id} onChange={e => {
                  ...
                  {sales.map(s => <option key={s.id} value={s.id}>{s.invoice_number} â€” {s.customer_name} ({fmt(s.total_value)})</option>)}
                </select>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
            ...
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, color:C.primary, marginBottom:14 }}>ðŸšš Transport Details</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, color:C.primary, marginBottom:14 }}>ðŸ“ From (Consignor)</div>
            ...
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, color:C.primary, marginBottom:14 }}>ðŸ“ To (Consignee)</div>
            ...
            <button style={{ ...btn("success"), width:"100%", justifyContent:"center", padding:"13px", fontSize:15 }} onClick={generate} disabled={loading}>
              {loading ? "â³ Generating E-Way Billâ€¦" : "ðŸšš Generate E-Way Bill"}
            </button>
            <div style={{ ...card, padding:16, background:"#FFF9E6" }}>
              <div style={{ fontWeight:700, fontSize:13, marginBottom:8 }}>âš ï¸ Remember:</div>
              <div style={{ fontSize:12, color:C.textMuted, lineHeight:1.8 }}>
                â€¢ Required for goods movement above â‚¹50,000<br/>
                â€¢ Valid for 1 day per 250 km for road transport<br/>
                â€¢ Must be generated before movement begins
              </div>
            </div>
          </div>
        </div>
...
// â”€â”€â”€ GOOGLE LOGIN HANDLER (added to AuthScreen) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Note: To enable Google login, go to Supabase â†’ Authentication â†’ Providers â†’ Google
...
function UploadPage({ data }) {
  ...
  async function doUpload(type) {
    ...
    try {
      ...
      setToast(`âœ… ${result.count} records saved to Supabase!`);
      setTimeout(() => setToast(""), 4000);
    } catch(e) { setToast("âŒ " + e.message); setTimeout(()=>setToast(""),5000); }
  }
  return (
    <div>
      ...
      <div style={{ fontSize:20, fontWeight:800, marginBottom:4 }}>ðŸ“¤ Upload GST Data</div>
      <div style={{ fontSize:13, color:C.textMuted, marginBottom:20 }}>Upload Excel/CSV â€” auto-saved to Supabase</div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        {[{type:"sales",label:"Sales Data",icon:"ðŸ“ˆ"},{type:"purchase",label:"Purchase Data",icon:"ðŸ“‰"}].map(({type,label,icon})=>(
              ...
              <>
                ...
                {files[type] && (
                  <div style={{ background:C.bg, borderRadius:8, padding:"10px 14px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    ...
                    <button style={btn()} onClick={()=>doUpload(type)} disabled={data.uploading}>{data.uploading?"Uploadingâ€¦":"Upload & Save"}</button>
                  </div>
                )}
              </>
            )}
...
function ReportsPage({ data, summary }) {
  ...
  return (
    <div>
      <div style={{ fontSize:20, fontWeight:800, marginBottom:4 }}>ðŸ“„ GST Reports</div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" }}>
        ...
        <button style={{ ...btn("outline"), marginLeft:"auto" }}>â¬‡ï¸ Download PDF</button>
      </div>
      <div style={card}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
...
function CalendarPage() {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800 }}>ðŸ“… Compliance Calendar</div>
          <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>FY {FY}-{(FY+1).toString().slice(2)} â€” Live GST & Tax Deadlines</div>
        </div>
        <button style={{ ...btn("outline"), fontSize:12 }} onClick={goToday}>ðŸ“ Today</button>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1.2fr 1fr", gap:20 }}>
        ...
        <div style={card}>
          {/* Month Navigation */}
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            <button onClick={prevMonth} style={{ background:"none", border:`1px solid ${C.border}`, borderRadius:7, padding:"6px 12px", cursor:"pointer", fontSize:16 }}>â€¹</button>
            ...
            <button onClick={nextMonth} style={{ background:"none", border:`1px solid ${C.border}`, borderRadius:7, padding:"6px 12px", cursor:"pointer", fontSize:16 }}>â€º</button>
          </div>
          ...
          {selectedDay && (
            <div style={{ marginTop:14, padding:"12px 14px", background:C.bg, borderRadius:8, border:`1px solid ${C.border}` }}>
              <div style={{ fontWeight:700, fontSize:13, marginBottom:8 }}>
                ...
                {dayDeadlines.length === 0 && <span style={{ color:C.textMuted, fontWeight:400, marginLeft:8 }}>â€” No deadlines</span>}
              </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                ...
                return (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:8, background:C.bg, border:`1px solid ${C.border}`, cursor:"pointer" }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:11, color:C.textMuted }}>{d.type} â€¢ {d.period}</div>
                    </div>
          ...
          {/* This Month Summary */}
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:12 }}>ðŸ“Š This Month</div>
...
function ClientsPage({ data, auth }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        <div><div style={{ fontSize:20, fontWeight:800 }}>ðŸ‘¥ Clients</div><div style={{ fontSize:13, color:C.textMuted }}>{data.clients.length} clients</div></div>
      ...
      <div style={card}>
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          ...
          <tbody>{data.clients.length===0?(
            <tr><td colSpan={8} style={{...TD,textAlign:"center",padding:30,color:C.textMuted}}>No clients yet â€” add your first client above</td></tr>
          ):data.clients.map((c,i)=>(
            <tr key={i}>
              <td style={{...TD,fontWeight:700}}>{c.company_name}</td>
              <td style={{...TD,fontFamily:"monospace",fontSize:11}}>{c.gstin||"â€”"}</td>
              <td style={TD}>{c.state}</td>
              <td style={TD}>{c.contact_name||"â€”"}</td>
              <td style={TD}>
                {c.mobile ? (
                ...
                ) : <span style={{ color:C.danger, fontSize:12 }}>âš ï¸ No mobile</span>}
              </td>
              <td style={TD}>{c.email||"â€”"}</td>
...
function SettingsPage({ auth }) {
  ...
  return (
    <div>
      <div style={{ fontSize:20, fontWeight:800, marginBottom:20 }}>âš™ï¸ Settings</div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:14, borderBottom:`1px solid ${C.border}`, paddingBottom:10 }}>ðŸ‘¤ Profile</div>
          ...
          <button style={btn()} onClick={async()=>{ await auth.updateProfile(pf); setSaved(true); setTimeout(()=>setSaved(false),2500); }}>{saved?"âœ“ Saved!":"Update Profile"}</button>
        </div>
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:14, borderBottom:`1px solid ${C.border}`, paddingBottom:10 }}>ðŸ¢ Company</div>
...
function ExpensePage({ data, auth }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
        <div><div style={{ fontSize:20, fontWeight:800 }}>ðŸ’¸ Expense Tracking</div><div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Track all business expenses with GST input credit</div></div>
        <button style={btn()} onClick={()=>setShow(true)}>+ Add Expense</button>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
        {[{ label:"Total Expenses", value:fmt(total), color:C.danger, icon:"ðŸ’¸" },{ label:"GST Input Credit", value:fmt(totalGST), color:C.success, icon:"ðŸ’°" },{ label:"This Month", value:fmt(expenses.filter(e=>e.date?.slice(0,7)===new Date().toISOString().slice(0,7)).reduce((a,e)=>a+Number(e.amount||0),0)), color:C.primary, icon:"ðŸ“…" },{ label:"Total Records", value:expenses.length, color:C.purple, icon:"ðŸ“‹" }].map((s,i)=>(
          <div key={i} style={{ ...card, padding:"16px 20px", borderTop:`3px solid ${s.color}` }}>
      ...
      {show && (
        <div style={{ ...card, marginBottom:20, background:C.primaryLighter, border:`1.5px solid ${C.primaryLight}40` }}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.primary }}>ðŸ“ New Expense</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:12 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr", gap:12, marginBottom:16 }}>
            ...
            <div><div style={lbl}>Amount (â‚¹)</div><input style={inp} type="number" placeholder="0" value={f.amount} onChange={e=>setF({...f,amount:e.target.value})} /></div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button style={btn()} onClick={save}>ðŸ’¾ Save Expense</button>
            <button style={btn("outline")} onClick={()=>setShow(false)}>Cancel</button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:20 }}>
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>ðŸ“‹ All Expenses</div>
            ...
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              ...
              <tbody>{expenses.map((e,i)=>(
                <tr key={i}>
                  ...
                  <td style={TD}>{e.vendor||"â€”"}</td>
                  <td style={{...TD, fontWeight:700, color:C.danger}}>{fmt(e.amount)}</td>
                  <td style={{...TD, color:C.success}}>{fmt(e.gst_amount)}</td>
                  <td style={TD}><button onClick={()=>del(e.id)} style={{ background:"none", border:"none", color:C.danger, cursor:"pointer", fontSize:16 }}>ðŸ—‘ï¸</button></td>
                </tr>
              ))}</tbody>
            </table>
          )}
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:12 }}>ðŸ“Š By Category</div>
          ...
          <div style={{ ...card, background:"#FFF9E6" }}>
            <div style={{ fontWeight:700, fontSize:13, marginBottom:8 }}>ðŸ’¡ GST Tip</div>
...
function PurchaseOrders({ auth, data }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
        <div><div style={{ fontSize:20, fontWeight:800 }}>ðŸ“¦ Purchase Orders</div><div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Manage vendor purchase orders end-to-end</div></div>
        <button style={btn()} onClick={()=>{ setShow(true); setView(null); }}>+ New Purchase Order</button>
      </div>
      {show && (
        <div style={{ ...card, marginBottom:20 }}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.primary }}>ðŸ“‹ New Purchase Order</div>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:12 }}>
            <thead><tr>{["Description","Qty","Rate (â‚¹)","GST %","Amount",""].map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead>
            <tbody>{f.items.map((l,i)=>(
              <tr key={i}>
                ...
                <td style={TD}><button onClick={()=>removeLine(i)} style={{ background:"none",border:"none",color:C.danger,cursor:"pointer" }}>âœ•</button></td>
              </tr>
            ))}</tbody>
          </table>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <div style={{ display:"flex", gap:10, marginTop:16 }}>
            <button style={btn("success")} onClick={savePO}>ðŸ’¾ Save PO</button>
            <button style={btn("outline")} onClick={()=>setShow(false)}>Cancel</button>
          </div>
        </div>
      )}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999, padding:20 }}>
          <div style={{ ...card, width:640, maxHeight:"85vh", overflowY:"auto" }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:16 }}>
              ...
              <button onClick={()=>setView(null)} style={{ background:"none", border:"none", fontSize:20, cursor:"pointer" }}>âœ•</button>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
              {[["Vendor",view.vendor],["Date",view.date],["Delivery",view.delivery_date||"â€”"],["GSTIN",view.vendor_gstin||"â€”"]].map(([k,v])=>(<div key={k}><div style={{ fontSize:11, color:C.textMuted, fontWeight:700 }}>{k}</div><div style={{ fontWeight:600 }}>{v}</div></div>))}
            </div>
        ...
        <table style={{ width:"100%", borderCollapse:"collapse" }}>
          ...
          <tbody>{orders.length===0?<tr><td colSpan={7} style={{...TD,textAlign:"center",padding:30,color:C.textMuted}}>No purchase orders yet</td></tr>:orders.map((o,i)=>(
            <tr key={i}>
              ...
              <td style={TD}>{o.delivery_date||"â€”"}</td>
...
// â”€â”€â”€ BANK RECONCILIATION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function BankReconciliation({ data }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ¦ Bank Reconciliation</div>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:20, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden", width:"fit-content" }}>
        {[["upload","ðŸ“¤ Upload Statement"],["reconcile","ðŸ”— Reconcile"],["matched","âœ… Matched"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} style={{ padding:"10px 20px", border:"none", background:tab===id?C.primary:"transparent", color:tab===id?C.white:C.textMuted, fontWeight:tab===id?700:500, fontSize:13, cursor:"pointer" }}>{label}</button>
        ))}
      </div>
      {tab==="upload" && (
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:16 }}>ðŸ“‚ Upload Bank Statement (CSV)</div>
          <div style={{ border:`2px dashed ${C.border}`, borderRadius:10, padding:"40px 20px", textAlign:"center", background:C.bg }}>
            ...
            <label style={{ ...btn(), display:"inline-flex", cursor:"pointer", padding:"11px 24px" }}>
              {uploading?"â³ Processingâ€¦":"ðŸ“ Choose CSV File"}
          ...
          <div style={{ marginTop:16, padding:"12px 16px", background:"#FFF9E6", borderRadius:8 }}>
            <div style={{ fontWeight:700, fontSize:13, marginBottom:6 }}>ðŸ’¡ How to export from your bank:</div>
            <div style={{ fontSize:12, color:C.textMuted, lineHeight:1.9 }}>â€¢ <strong>SBI:</strong> NetBanking â†’ Account Summary â†’ Download â†’ CSV<br/>â€¢ <strong>HDFC:</strong> NetBanking â†’ My Accounts â†’ Account Statement â†’ Download CSV<br/>â€¢ <strong>ICICI:</strong> iMobile/NetBanking â†’ Accounts â†’ Statement â†’ CSV format</div>
          </div>
        </div>
      )}
      {tab==="reconcile" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>ðŸ¦ Bank Transactions ({unmatchedBank.length} unmatched)</div>
            <div style={{ maxHeight:500, overflowY:"auto" }}>
              {unmatchedBank.length===0?<div style={{ textAlign:"center",padding:30,color:C.success }}>ðŸŽ‰ All transactions matched!</div>:unmatchedBank.map((t,i)=>(
          ...
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>âœ… Matched ({matchedCount})</div>
            <div style={{ maxHeight:500, overflowY:"auto" }}>
                ...
                return (
                  <div key={i} style={{ padding:"10px 12px", border:`1px solid ${C.success}40`, borderRadius:7, marginBottom:8, background:C.successLight }}>
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:12 }}>
                      <span style={{ color:C.success, fontWeight:700 }}>âœ“ Matched</span>
                    ...
                    <div style={{ fontSize:12, marginTop:4 }}>{bank.description.slice(0,40)} â†” {inv._ref} ({inv._party})</div>
                  </div>
                );
              })}
            </div>
            ...
            <tbody>{bankTxns.map((t,i)=>{
              ...
              return (
                <tr key={i}>
                  ...
                  <td style={TD}>{inv?inv._ref:"â€”"}</td>
                  <td style={TD}>{inv?inv._party:"â€”"}</td>
...
// â”€â”€â”€ GSTR-2B RECONCILIATION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function GSTR2BReconciliation({ data }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ“‘ GSTR-2B Reconciliation</div>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:20, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden", width:"fit-content" }}>
        {[["upload","ðŸ“¤ Upload GSTR-2B"],["reconcile","ðŸ”— Reconcile"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} style={{ padding:"10px 20px", border:"none", background:tab===id?C.primary:"transparent", color:tab===id?C.white:C.textMuted, fontWeight:tab===id?700:500, fontSize:13, cursor:"pointer" }}>{label}</button>
        ))}
      </div>
      {tab==="upload" && (
        <div style={card}>
          <div style={{ border:`2px dashed ${C.border}`, borderRadius:10, padding:"40px 20px", textAlign:"center", background:C.bg, marginBottom:20 }}>
            ...
            <div style={{ color:C.textMuted, fontSize:13, marginBottom:20 }}>Download from GST Portal â†’ Returns â†’ GSTR-2B â†’ Download JSON</div>
            <label style={{ ...btn(), display:"inline-flex", cursor:"pointer" }}>
              ðŸ“ Upload GSTR-2B JSON
          ...
          <div style={{ padding:"12px 16px", background:"#EBF5FB", borderRadius:8 }}>
            <div style={{ fontWeight:700, fontSize:13, color:C.primary, marginBottom:6 }}>ðŸ“ How to download GSTR-2B:</div>
            <div style={{ fontSize:12, color:C.textMuted, lineHeight:2 }}>1. Login to <strong>gst.gov.in</strong><br/>2. Services â†’ Returns â†’ GSTR-2B<br/>3. Select month â†’ Click "Download"<br/>4. Upload the JSON file here</div>
          </div>
        </div>
      )}
      {tab==="reconcile" && (
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>{matchedPairs.length===0?<tr><td colSpan={9} style={{...TD,textAlign:"center",padding:30,color:C.textMuted}}>Upload GSTR-2B to begin reconciliation</td></tr>:matchedPairs.map((g,i)=>(
              <tr key={i} style={{ background:g.matched?"transparent":"#FFF5F5" }}>
                ...
                <td style={TD}>{g.matched?<span style={{ color:C.success }}>âœ… {g.matchedBill?.bill_number}</span>:<span style={{ color:C.danger }}>âŒ Not found</span>}</td>
...
// â”€â”€â”€ TDS MANAGER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TDSManager({ auth }) {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
        <div><div style={{ fontSize:20, fontWeight:800 }}>ðŸ“‹ TDS Manager</div><div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Track TDS deductions, deposits and Form 26AS</div></div>
      ...
      {show && (
        <div style={{ ...card, marginBottom:20, background:C.primaryLighter }}>
          <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.primary }}>ðŸ“‹ New TDS Entry</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:12 }}>
            <div><div style={lbl}>Date</div><input style={inp} type="date" value={f.date} onChange={e=>setF({...f,date:e.target.value})} /></div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:12, marginBottom:16 }}>
            <div><div style={lbl}>Section</div><select style={inp} value={f.section} onChange={e=>{ const s=SECTIONS.find(x=>x.code===e.target.value); setF({...f,section:e.target.value,payment_nature:s?.nature||"",tds_rate:s?.rate||0}); }}>{SECTIONS.map(s=><option key={s.code} value={s.code}>Sec {s.code} â€” {s.nature}</option>)}</select></div>
            <div><div style={lbl}>Payment Amount (â‚¹)</div><input style={inp} type="number" value={f.amount} onChange={e=>setF({...f,amount:e.target.value})} /></div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button style={btn()} onClick={save}>ðŸ’¾ Save</button>
            <button style={btn("outline")} onClick={()=>setShow(false)}>Cancel</button>
          </div>
        </div>
      )}
...
// â”€â”€â”€ P&L AND BALANCE SHEET â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function FinancialReports({ data, auth }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ“Š Financial Reports</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>P&L Statement, Balance Sheet & Tax Summary â€” FY {new Date().getMonth()>=3?new Date().getFullYear():new Date().getFullYear()-1}-{(new Date().getMonth()>=3?new Date().getFullYear()+1:new Date().getFullYear()).toString().slice(2)}</div>
      </div>
      <div style={{ display:"flex", gap:0, marginBottom:20, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden", width:"fit-content" }}>
        {[["pl","ðŸ“ˆ P&L Statement"],["balance","âš–ï¸ Balance Sheet"],["tax","ðŸ§¾ Tax Summary"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} style={{ padding:"10px 20px", border:"none", background:tab===id?C.primary:"transparent", color:tab===id?C.white:C.textMuted, fontWeight:tab===id?700:500, fontSize:13, cursor:"pointer" }}>{label}</button>
        ))}
      </div>
      {tab==="pl" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.primary }}>ðŸ“ˆ Profit & Loss Statement</div>
            {[
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:12 }}>ðŸ“Š Revenue Breakdown</div>
            ...
            <div style={{ ...card, background:netProfit>=0?C.successLight:"#FFF5F5" }}>
              <div style={{ fontWeight:800, fontSize:16, color:netProfit>=0?C.success:C.danger }}>{netProfit>=0?"âœ… Profitable Business":"âš ï¸ Running at Loss"}</div>
      ...
      {tab==="balance" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.success }}>ðŸ“¥ ASSETS</div>
          ...
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.danger }}>ðŸ“¤ LIABILITIES & EQUITY</div>
      ...
      {tab==="tax" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.primary }}>ðŸ§¾ GST Summary</div>
          ...
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.primary }}>ðŸ“‹ Income Tax Estimate</div>
...
function AuditTrail({ auth }) {
  const [logs] = useState(() => {
    try {
      ...
      if (stored.length === 0) {
        return [
          { id:1, action:"Invoice Created", module:"Invoices", user: auth.profile?.name||auth.user?.email||"User", details:"Invoice INV-001 created for Reliance Industries â€” â‚¹2,95,000", time: new Date(Date.now()-3600000).toLocaleString() },
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ” Audit Trail</div>
        ...
        {filtered.length===0?<div style={{ textAlign:"center",padding:40,color:C.textMuted }}>No audit logs for this module</div>:filtered.map((log,i)=>(
          <div key={i} style={{ display:"flex", gap:16, padding:"14px 0", borderBottom:`1px solid ${C.border}` }}>
            <div style={{ width:40, height:40, borderRadius:"50%", background:(moduleColor[log.module]||C.primary)+"22", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>
              {log.module==="Invoices"?"ðŸ§¾":log.module==="Reports"?"ðŸ“„":log.module==="Auth"?"ðŸ”":log.module==="Settings"?"âš™ï¸":log.module==="Expenses"?"ðŸ’¸":log.module==="TDS"?"ðŸ“‹":"ðŸ¦"}
            </div>
            <div style={{ flex:1 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              ...
              <div style={{ fontSize:11, color:C.textMuted, marginTop:2 }}>ðŸ‘¤ {log.user}</div>
            </div>
          </div>
        ))}
      </div>
...
function ClientPortal({ data, auth }) {
  ...
  function copyLink() {
    ...
    alert("âœ… Link copied! Share with your client via WhatsApp or Email.");
  }
  const clientInvoices = selectedClient ? sales.filter(s=>s.customer_name===selectedClient.company_name) : [];
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ”— Client Portal</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Share invoice links with clients â€” they can view & download without login</div>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>ðŸ‘¥ Select Client to Share</div>
          {clients.length===0?<div style={{ textAlign:"center",padding:30,color:C.textMuted }}>No clients found. Add clients first.</div>:clients.map((c,i)=>(
            <div key={i} onClick={()=>generateLink(c)} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 14px", border:`1.5px solid ${selectedClient?.id===c.id?C.primary:C.border}`, borderRadius:8, marginBottom:8, cursor:"pointer", background:selectedClient?.id===c.id?C.primaryLighter:C.white }}>
              <div>
                ...
                <div style={{ fontSize:11, color:C.textMuted }}>{c.gstin||"No GSTIN"} â€¢ {c.state}</div>
              </div>
              <div style={{ fontSize:12, color:C.primary, fontWeight:600 }}>ðŸ”— Generate Link</div>
            </div>
          ))}
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          {selectedClient && (
            <>
              <div style={{ ...card, background:C.primaryLighter, border:`1.5px solid ${C.primaryLight}40` }}>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:12, color:C.primary }}>ðŸ”— Share Link for {selectedClient.company_name}</div>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button style={btn()} onClick={copyLink}>ðŸ“‹ Copy Link</button>
                  <button style={btn("success")} onClick={()=>{ const msg=`Dear ${selectedClient.company_name},\n\nPlease find your invoices here:\n${shareLink}\n\nRegards,\n${auth.activeCompany?.company_name}`; window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`,"_blank"); }}>ðŸ’¬ Share on WhatsApp</button>
                </div>
              </div>
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:12 }}>ðŸ§¾ Invoices for {selectedClient.company_name}</div>
...
// â”€â”€â”€ PAYMENT GATEWAY (RAZORPAY) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function PaymentGateway({ data, auth }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ’³ Payment Collection</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Create payment links via Razorpay â€” collect payments instantly</div>
      </div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        <div style={{ display:"flex", gap:0, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden" }}>
          {[["links","ðŸ’³ Payment Links"],["settings","âš™ï¸ Settings"]].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)} style={{ padding:"10px 20px", border:"none", background:tab===id?C.primary:"transparent", color:tab===id?C.white:C.textMuted, fontWeight:tab===id?700:500, fontSize:13, cursor:"pointer" }}>{label}</button>
          ))}
        </div>
      ...
      {show && (
        <div style={{ ...card, marginBottom:20, background:C.primaryLighter }}>
          <div style={{ fontWeight:700,fontSize:15,marginBottom:16,color:C.primary }}>ðŸ’³ New Payment Link</div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
            ...
            <div><div style={lbl}>Amount (â‚¹)</div><input style={inp} type="number" placeholder="10000" value={f.amount} onChange={e=>setF({...f,amount:e.target.value})} /></div>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button style={btn("success")} onClick={createLink}>ðŸ”— Generate Payment Link</button>
            <button style={btn("outline")} onClick={()=>setShow(false)}>Cancel</button>
          </div>
        </div>
      )}
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>{links.length===0?<tr><td colSpan={7} style={{...TD,textAlign:"center",padding:30,color:C.textMuted}}>No payment links yet</td></tr>:links.map((l,i)=>(
              <tr key={i}>
                ...
                <td style={TD}>{l.customer_email||l.customer_mobile||"â€”"}</td>
      ...
      {tab==="settings" && (
        <div style={card}>
          <div style={{ fontWeight:700,fontSize:15,marginBottom:16 }}>âš™ï¸ Razorpay Configuration</div>
          <div style={{ padding:"20px", background:"#FFF9E6", borderRadius:8, marginBottom:20 }}>
            <div style={{ fontWeight:700,fontSize:13,marginBottom:8 }}>âš ï¸ Setup Required</div>
          ...
          <button style={{ ...btn("success"), marginTop:16 }}>ðŸ’¾ Save Configuration</button>
        </div>
      )}
    </div>
  );
...
// â”€â”€â”€ TALLY IMPORT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TallyImport({ data }) {
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ“¥ Tally / Busy Import</div>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:24, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden", width:"fit-content" }}>
        {[1,2,3].map(s=>(
          <div key={s} style={{ padding:"10px 24px", background:step===s?C.primary:step>s?C.success:"transparent", color:step>=s?C.white:C.textMuted, fontWeight:700, fontSize:13 }}>
            {step>s?"âœ“ ":""}{s===1?"Upload File":s===2?"Review Data":"Import Complete"}
          </div>
        ))}
      </div>
      {step===1 && (
        <div style={card}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:24 }}>
            {[{ name:"Tally ERP 9 / Prime", icon:"ðŸ“Š", format:"XML or CSV", steps:["Tally â†’ Gateway â†’ Display â†’ Day Book","Press Alt+E to export","Choose Excel/CSV format","Upload here"] },{ name:"Busy Accounting", icon:"ðŸ“‹", format:"Excel/CSV", steps:["Reports â†’ Day Book","Export â†’ Excel","Save as CSV","Upload here"] },{ name:"Marg ERP", icon:"ðŸ“ˆ", format:"CSV", steps:["Reports â†’ Export","Choose CSV format","Save file","Upload here"] },{ name:"Any Other Software", icon:"ðŸ“", format:"CSV", steps:["Export transactions as CSV","Columns: Type, Date, Party, Amount","Save file","Upload here"] }].map((s,i)=>(
          ...
          <div style={{ border:`2px dashed ${C.border}`, borderRadius:10, padding:"40px", textAlign:"center", background:C.bg }}>
            ...
            <label style={{ ...btn(), display:"inline-flex", cursor:"pointer", padding:"11px 24px" }}>
              {loading?"â³ Processingâ€¦":"ðŸ“ Choose File (.xml, .csv, .xlsx)"}
      ...
      {step===2 && imported && (
        <div>
          <div style={{ ...card, marginBottom:20, background:C.successLight, border:`1px solid ${C.success}40` }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div><div style={{ fontWeight:800,fontSize:16,color:C.success }}>âœ… {imported.count} Records Ready to Import</div><div style={{ fontSize:13,color:C.textMuted,marginTop:4 }}>Source: {imported.source==="tally_xml"?"Tally XML Export":"CSV/Excel"}</div></div>
              <div style={{ display:"flex", gap:10 }}>
                <button style={btn("success")} onClick={()=>setStep(3)}>âœ… Confirm Import</button>
                <button style={btn("outline")} onClick={()=>setStep(1)}>â† Back</button>
              </div>
            </div>
          </div>
          <div style={card}>
            ...
            <table style={{ width:"100%",borderCollapse:"collapse" }}>
              ...
              <tbody>{imported.records.slice(0,10).map((r,i)=>(
                <tr key={i}><td style={TD}>{i+1}</td><td style={TD}><span style={badge(C.primary)}>{r.type}</span></td><td style={TD}>{r.date}</td><td style={{...TD,fontWeight:600}}>{r.party}</td><td style={{...TD,fontWeight:700}}>{fmt(Math.abs(r.amount))}</td><td style={TD}>{r.narration?.slice(0,30)||"â€”"}</td></tr>
              ))}</tbody>
            </table>
          </div>
        </div>
...
// â”€â”€â”€ INVENTORY MANAGEMENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function InventoryPage({ data }) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div><div style={{fontSize:20,fontWeight:800}}>ðŸ“¦ Inventory Management</div><div style={{fontSize:13,color:C.textMuted,marginTop:4}}>Track stock levels, auto-update on sales, set reorder alerts</div></div>
        <button style={btn()} onClick={()=>setShow(true)}>+ Add Item</button>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:20}}>
        {[{label:"Total Items",value:items.length,color:C.primary,icon:"ðŸ“¦"},{label:"Low Stock Alerts",value:lowStock.length,color:C.danger,icon:"âš ï¸"},{label:"Total Stock Value",value:fmt(totalValue),color:C.success,icon:"ðŸ’°"},{label:"Out of Stock",value:items.filter(i=>i.current_stock===0).length,color:C.warning,icon:"ðŸš«"}].map((s,i)=>(
          <div key={i} style={{...card,padding:"16px 20px",borderTop:`3px solid ${s.color}`}}>
      ...
      {lowStock.length>0&&<div style={{...card,marginBottom:16,background:"#FFF5F5",border:`1px solid ${C.danger}30`}}><div style={{fontWeight:700,fontSize:13,color:C.danger,marginBottom:8}}>âš ï¸ Low Stock Alert â€” {lowStock.length} items need reordering</div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{lowStock.map((i,idx)=><span key={idx} style={{...badge(C.danger),fontSize:12}}>{i.name}: {i.current_stock} {i.unit} left</span>)}</div></div>}
      {show&&(
        <div style={{...card,marginBottom:20,background:C.primaryLighter}}>
          <div style={{fontWeight:700,fontSize:15,marginBottom:16,color:C.primary}}>ðŸ“¦ Add New Item</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:16}}>
            ...
            <div><div style={lbl}>Purchase Price (â‚¹)</div><input style={inp} type="number" value={f.purchase_price} onChange={e=>setF({...f,purchase_price:e.target.value})} /></div>
            <div><div style={lbl}>Selling Price (â‚¹)</div><input style={inp} type="number" value={f.selling_price} onChange={e=>setF({...f,selling_price:e.target.value})} /></div>
          </div>
          <div style={{display:"flex",gap:10}}><button style={btn("success")} onClick={save}>ðŸ’¾ Save Item</button><button style={btn("outline")} onClick={()=>setShow(false)}>Cancel</button></div>
        </div>
      )}
      {adjItem&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999}}>
          <div style={{...card,width:400,padding:32}}>
            <div style={{fontWeight:700,fontSize:16,marginBottom:16}}>Adjust Stock â€” {adjItem.name}</div>
      ...
      <div style={{display:"flex",gap:0,marginBottom:16,background:C.white,borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden",width:"fit-content"}}>
        {[["stock","ðŸ“¦ Stock List"],["movement","ðŸ“Š Stock Movement"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} style={{padding:"10px 20px",border:"none",background:tab===id?C.primary:"transparent",color:tab===id?C.white:C.textMuted,fontWeight:tab===id?700:500,fontSize:13,cursor:"pointer"}}>{label}</button>
        ))}
      </div>
      {tab==="stock"&&<div style={card}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["Item","SKU","Category","Unit","Stock","Reorder","Purchase â‚¹","Selling â‚¹","Value","Status","Action"].map(h=><th key={h} style={TH}>{h}</th>)}</tr></thead><tbody>{items.length===0?<tr><td colSpan={11} style={{...TD,textAlign:"center",padding:30,color:C.textMuted}}>No items added yet</td></tr>:items.map((item,i)=>(
        <tr key={i} style={{background:item.current_stock===0?"#FFF5F5":item.current_stock<=item.reorder_level?"#FFF9E6":"transparent"}}>
          ...
          <td style={{...TD,fontSize:11,fontFamily:"monospace"}}>{item.sku||"â€”"}</td>
          <td style={TD}>{item.category||"â€”"}</td>
          ...
          <td style={TD}><div style={{display:"flex",gap:6}}><button style={{...btn("outline"),fontSize:11,padding:"4px 8px"}} onClick={()=>setAdjItem(item)}>Adjust</button><button onClick={()=>del(item.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer"}}>ðŸ—‘ï¸</button></div></td>
        </tr>
      ))}</tbody></table></div>}
...
// â”€â”€â”€ MULTI-USER / TEAM ACCESS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TeamAccess({ auth }) {
  ...
  const ROLES = [
    { id:"Owner", label:"Owner", desc:"Full access â€” all modules, billing, settings", color:C.purple, perms:["all"] },
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div><div style={{fontSize:20,fontWeight:800}}>ðŸ‘¥ Team Access</div><div style={{fontSize:13,color:C.textMuted,marginTop:4}}>Invite team members with role-based permissions</div></div>
        <button style={btn()} onClick={()=>setShow(true)}>+ Invite Member</button>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20}}>
        <div style={card}>
          <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>ðŸ‘¤ Current Team ({members.length+1} members)</div>
          ...
          {members.map((m,i)=>(
            <div key={i} style={{display:"flex",gap:12,alignItems:"center",padding:"12px 0",borderBottom:`1px solid ${C.border}`}}>
              ...
              <button onClick={()=>remove(m.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:14}}>âœ•</button>
            </div>
          ))}
        ...
        <div style={card}>
          <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>ðŸ” Role Permissions</div>
          {ROLES.map((role,i)=>(
      ...
      {show&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:20}}>
          <div style={{...card,width:520,padding:32}}>
            <div style={{fontWeight:800,fontSize:17,marginBottom:4,color:C.primary}}>ðŸ“§ Invite Team Member</div>
            ...
            <div style={{display:"flex",gap:10}}><button style={btn("success")} onClick={invite} disabled={!f.email}>ðŸ“§ Send Invitation</button><button style={btn("outline")} onClick={()=>setShow(false)}>Cancel</button></div>
          </div>
        </div>
      )}
    </div>
...
// â”€â”€â”€ RECURRING INVOICES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function RecurringInvoices({ auth, data }) {
  ...
  function generateNow(id) {
    ...
    setRecurring(updated); localStorage.setItem("ts_recurring",JSON.stringify(updated));
    alert("âœ… Invoice generated! Check Invoice Generator for the new invoice.");
  }
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div><div style={{fontSize:20,fontWeight:800}}>ðŸ” Recurring Invoices</div><div style={{fontSize:13,color:C.textMuted,marginTop:4}}>Auto-generate invoices for subscription clients every month</div></div>
      ...
      {show&&(
        <div style={{...card,marginBottom:20,background:C.primaryLighter}}>
          <div style={{fontWeight:700,fontSize:15,marginBottom:16,color:C.primary}}>ðŸ” New Recurring Invoice</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
            ...
            <div><div style={lbl}>Amount (â‚¹, excl. GST)</div><input style={inp} type="number" placeholder="10000" value={f.amount} onChange={e=>setF({...f,amount:e.target.value})} /></div>
            <div><div style={lbl}>GST Rate (%)</div><select style={inp} value={f.gst_rate} onChange={e=>setF({...f,gst_rate:Number(e.target.value)})}>{[0,5,12,18,28].map(r=><option key={r} value={r}>{r}%</option>)}</select></div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:10}}><button style={btn("success")} onClick={save}>ðŸ’¾ Save Schedule</button><button style={btn("outline")} onClick={()=>setShow(false)}>Cancel</button></div>
        </div>
      )}
      <div style={card}>
        {recurring.length===0?<div style={{textAlign:"center",padding:40,color:C.textMuted}}><div style={{fontSize:40,marginBottom:12}}>ðŸ”</div><div style={{fontWeight:700,fontSize:15}}>No recurring invoices yet</div><div style={{marginTop:8}}>Set up recurring invoices for retainer clients, subscriptions, or monthly services</div></div>:(
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{recurring.map((r,i)=>(
              <tr key={i}>
                ...
                <td style={TD}>{r.description||"â€”"}</td>
                ...
                <td style={TD}>
                  <div style={{display:"flex",gap:6}}>
                    ...
                    <button onClick={()=>del(r.id)} style={{background:"none",border:"none",color:C.danger,cursor:"pointer"}}>ðŸ—‘ï¸</button>
                  </div>
                </td>
              </tr>
            ))}</tbody>
...
// â”€â”€â”€ GST HEALTH CHECK â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function GSTHealthCheck({ auth, data }) {
    ...
    setTimeout(()=>{
      ...
      setResults({
        ...
        issues: v.valid ? [
          ...
          { type:"success", msg:"ITC claim matches GSTR-2B â€” no mismatch" },
  ...
  return (
    <div>
      <div style={{marginBottom:20}}>
        <div style={{fontSize:20,fontWeight:800}}>ðŸ¥ GST Health Check</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        <div>
          <div style={card}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:14,color:C.primary}}>ðŸ” Check Your GSTIN Health</div>
            <div style={{display:"flex",gap:10,marginBottom:16}}>
              ...
              <button style={{...btn(),padding:"10px 20px"}} onClick={checkHealth} disabled={loading}>{loading?"ðŸ” Checkingâ€¦":"Check Now"}</button>
            </div>
            {results&&(
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",background:results.valid?C.successLight:"#FFF5F5",borderRadius:8,marginBottom:16,border:`1px solid ${results.valid?C.success+"40":C.danger+"40"}`}}>
                  <div>
                    <div style={{fontWeight:800,fontSize:16,color:results.valid?C.success:C.danger}}>{results.valid?"âœ… GSTIN Valid & Active":"âŒ Invalid GSTIN"}</div>
                  ...
                  <>
                    ...
                    {results.issues.map((issue,i)=>(
                      <div key={i} style={{display:"flex",gap:10,padding:"8px 12px",borderRadius:6,marginBottom:6,background:issue.type==="error"||issue.type==="warning"?"#FFF9E6":issue.type==="success"?C.successLight:C.primaryLighter}}>
                        <span>{issue.type==="success"?"âœ…":issue.type==="warning"?"âš ï¸":issue.type==="info"?"â„¹ï¸":"âŒ"}</span>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div style={card}>
            <div style={{fontWeight:700,fontSize:14,marginBottom:14,color:C.primary}}>ðŸ”Ž Verify Client / Supplier GSTIN</div>
            ...
            {clientResults.map((r,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",borderRadius:6,marginBottom:6,background:r.valid?C.successLight:"#FFF5F5"}}>
                ...
                <span style={badge(r.valid?C.success:C.danger)}>{r.valid?"âœ“ Valid":"âœ— Invalid"}</span>
              </div>
            ))}
          ...
          <div style={{...card,background:"#FFF9E6"}}>
            <div style={{fontWeight:700,fontSize:13,marginBottom:8}}>âš ï¸ GSTIN Format Guide</div>
            <div style={{fontSize:12,color:C.textMuted,lineHeight:2}}>
              Format: <strong>22AAAAA0000A1Z5</strong><br/>
              â€¢ First 2 digits = State code (e.g. 27=Maharashtra)<br/>
              â€¢ Next 10 = PAN of the taxpayer<br/>
              â€¢ 13th = Entity number (1-9)<br/>
              â€¢ 14th = Always Z<br/>
              â€¢ 15th = Check digit
            </div>
          </div>
          <div style={card}>
            <div style={{fontWeight:700,fontSize:13,marginBottom:12}}>ðŸ“Š Compliance Checklist</div>
            {[["GSTR-1 filed monthly","âœ…"],["GSTR-3B filed monthly","âœ…"],["ITC matched with GSTR-2B","âœ…"],["GSTR-9 filed annually","âš ï¸"],["E-invoicing enabled (if turnover>5Cr)","â„¹ï¸"],["HSN codes in invoices","âœ…"]].map(([task,status],i)=>(
...
// â”€â”€â”€ PAYROLL MANAGEMENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function PayrollPage({ auth }) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div><div style={{fontSize:20,fontWeight:800}}>ðŸ’¼ Payroll Management</div><div style={{fontSize:13,color:C.textMuted,marginTop:4}}>Manage salaries, PF, ESI, TDS deductions and generate payslips</div></div>
        <div style={{display:"flex",gap:10}}><button style={btn("success")} onClick={runPayroll} disabled={employees.length===0}>â–¶ Run Payroll</button><button style={btn()} onClick={()=>setShow(true)}>+ Add Employee</button></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:20}}>
      ...
      {show&&(
        <div style={{...card,marginBottom:20,background:C.primaryLighter}}>
          <div style={{fontWeight:700,fontSize:15,marginBottom:16,color:C.primary}}>ðŸ‘¤ Add Employee</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
            ...
            <div><div style={lbl}>Designation</div><input style={inp} placeholder="Manager, Developerâ€¦" value={f.designation} onChange={e=>setF({...f,designation:e.target.value})} /></div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:12,marginBottom:12}}>
            {[["Basic (â‚¹)","basic"],["HRA (â‚¹)","hra"],["Allowances (â‚¹)","allowances"],["PF Employee (%)","pf_employee"],["ESI (%)","esi"],["TDS/Month (â‚¹)","tds_monthly"]].map(([label,key])=>(
      ...
      <div style={card}>
        ...
        {tab==="pf"&&<div style={{padding:20}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:20}}>{[{label:"Total Employee PF",value:fmt(employees.reduce((a,e)=>a+calcNetSalary(e).pf,0)),color:C.primary},{label:"Total Employer PF",value:fmt(employees.reduce((a,e)=>a+calcNetSalary(e).employer_pf,0)),color:C.success},{label:"Total ESI",value:fmt(employees.reduce((a,e)=>a+calcNetSalary(e).esi,0)),color:C.warning}].map((s,i)=><div key={i} style={{...card,padding:16,borderTop:`3px solid ${s.color}`}}><div style={{fontSize:11,color:C.textMuted,fontWeight:700,textTransform:"uppercase",marginBottom:4}}>{s.label}</div><div style={{fontSize:18,fontWeight:800,color:s.color}}>{s.value}</div></div>)}</div><div style={{padding:"12px 16px",background:"#FFF9E6",borderRadius:8}}><div style={{fontWeight:700,fontSize:13,marginBottom:8}}>ðŸ“… PF/ESI Deposit Deadlines</div><div style={{fontSize:12,color:C.textMuted,lineHeight:2}}>â€¢ <strong>PF Deposit:</strong> 15th of every month<br/>â€¢ <strong>ESI Deposit:</strong> 15th of every month<br/>â€¢ <strong>PF Return (ECR):</strong> 25th of every month<br/>â€¢ <strong>ESI Return:</strong> 11th of April and October</div></div></div>}
      </div>
    </div>
  );
}

// â”€â”€â”€ FORM 16 GENERATOR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€


// â”€â”€â”€ ENHANCED FORM 16 GENERATOR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function Form16Generator({ auth }) {
  ...
  function generateForm16(emp) {
    ...
    return {
      ...
      employer_gstin: auth.activeCompany?.gstin || "â€”",
...
Designation                 : ${data.emp.designation || "â€”"}
Period of Employment        : April ${data.fy.split("-")[0]} to March ${data.ay.split("-")[0]}

Total TDS Deducted and Deposited : â‚¹${data.tds_deducted.toLocaleString("en-IN")}

PART B â€” COMPUTATION OF INCOME
--------------------------------------------------------------------------------
GROSS SALARY
  Basic Salary (12 months)     : â‚¹${(Number(data.emp.basic) * 12).toLocaleString("en-IN")}
  House Rent Allowance         : â‚¹${(Number(data.emp.hra) * 12).toLocaleString("en-IN")}
  Other Allowances             : â‚¹${(Number(data.emp.allowances) * 12).toLocaleString("en-IN")}
                                 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  GROSS SALARY                 : â‚¹${data.annual.toLocaleString("en-IN")}

DEDUCTIONS UNDER CHAPTER VI-A
  Provident Fund (Sec 80C)     : â‚¹${Math.round(data.pf).toLocaleString("en-IN")}
  Standard Deduction (Sec 16)  : â‚¹${data.std_deduction.toLocaleString("en-IN")}
                                 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  TOTAL DEDUCTIONS             : â‚¹${Math.round(data.pf + data.std_deduction).toLocaleString("en-IN")}

NET TAXABLE INCOME             : â‚¹${Math.round(data.taxable).toLocaleString("en-IN")}

TAX COMPUTATION
  Income Tax                   : â‚¹${data.tax.toLocaleString("en-IN")}
  Health & Education Cess (4%) : â‚¹${data.cess.toLocaleString("en-IN")}
                                 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  TOTAL TAX LIABILITY          : â‚¹${data.total_tax.toLocaleString("en-IN")}
  Less: TDS Already Deducted   : â‚¹${data.tds_deducted.toLocaleString("en-IN")}
                                 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ${data.refund > 0 ? "REFUND DUE              " : "BALANCE TAX PAYABLE     "} : â‚¹${(data.refund || data.payable).toLocaleString("en-IN")}
...
================================================================================
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}>
        <div>
          <div style={{ fontSize:20, fontWeight:800 }}>ðŸ“‹ Form 16 Generator</div>
          <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Generate Form 16 (TDS certificate) for employees â€” Part A & Part B</div>
        </div>
        {employees.length > 0 && <button style={btn("success")} onClick={generateAll} disabled={generating}>{generating ? "â³ Generatingâ€¦" : `ðŸ“¥ Generate All (${employees.length})`}</button>}
      </div>
      {employees.length === 0 ? (
        <div style={{ ...card, textAlign:"center", padding:60 }}>
          <div style={{ fontSize:48, marginBottom:16 }}>ðŸ“‹</div>
          ...
          <div style={{ padding:"12px 20px", background:"#FFF9E6", borderRadius:8, fontSize:13, color:C.warning, display:"inline-block" }}>ðŸ’¼ Go to Payroll â†’ Add Employee</div>
        </div>
      ) : (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1.2fr", gap:20 }}>
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>âš™ï¸ Generate Settings</div>
              ...
              {selected && <button style={{ ...btn("success"), width:"100%", justifyContent:"center", padding:"12px" }} onClick={() => { const emp = employees.find(x => x.id === selected); if(emp) downloadForm16(generateForm16(emp)); }}>ðŸ“¥ Download Form 16</button>}
            </div>
            <div style={{ ...card, background:"#FFF9E6" }}>
              <div style={{ fontWeight:700, fontSize:13, marginBottom:8 }}>ðŸ“Œ About Form 16</div>
              <div style={{ fontSize:12, color:C.textMuted, lineHeight:2 }}>
                â€¢ Issued by employer to employee annually<br/>
                â€¢ Required for ITR filing by employee<br/>
                â€¢ Deadline: <strong>15th June</strong> every year<br/>
                â€¢ Contains Part A (TDS details) + Part B (salary)<br/>
                â€¢ Must be digitally signed for official use (DSC)
              </div>
            </div>
          </div>
          {generated ? (
            <div style={{ ...card, fontFamily:"monospace", fontSize:12, lineHeight:1.9, background:"#F8F9FA", maxHeight:520, overflowY:"auto" }}>
              <div style={{ background:C.primary, color:C.white, padding:"8px 16px", borderRadius:"6px 6px 0 0", fontWeight:700, fontSize:13, marginBottom:12 }}>Preview â€” Form 16 for {generated.emp.name} (FY {generated.fy})</div>
              <div style={{ padding:"0 16px 16px" }}>
                {[["Employee", generated.emp.name], ["PAN", generated.emp.pan || "Not Provided"], ["Gross Salary", `â‚¹${generated.annual.toLocaleString("en-IN")}`], ["PF Deduction", `â‚¹${Math.round(generated.pf).toLocaleString("en-IN")}`], ["Standard Deduction", "â‚¹50,000"], ["Taxable Income", `â‚¹${Math.round(generated.taxable).toLocaleString("en-IN")}`], ["Income Tax", `â‚¹${generated.tax.toLocaleString("en-IN")}`], ["Cess (4%)", `â‚¹${generated.cess.toLocaleString("en-IN")}`], ["Total Tax", `â‚¹${generated.total_tax.toLocaleString("en-IN")}`], ["TDS Deducted", `â‚¹${generated.tds_deducted.toLocaleString("en-IN")}`], [generated.refund > 0 ? "Refund Due" : "Balance Payable", `â‚¹${(generated.refund || generated.payable).toLocaleString("en-IN")}`]].map(([k,v], i) => (
      ...
      {employees.length > 0 && (
        <div style={{ ...card, marginTop:20 }}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>ðŸ“Š All Employees â€” Form 16 Status</div>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>{employees.map((emp, i) => {
              ...
              return (
                <tr key={i}>
                  ...
                  <td style={TD}>{emp.designation || "â€”"}</td>
                  ...
                  <td style={TD}><button style={{ ...btn("success"), fontSize:11, padding:"4px 10px" }} onClick={() => downloadForm16(d)}>ðŸ“¥ Download</button></td>
                </tr>
              );
            })}</tbody>
          </table>
...
// â”€â”€â”€ e-TDS / FVU FILING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function ETDSFiling({ auth }) {
  ...
  function generateFVU(formType) {
    ...
    setTimeout(() => {
      ...
      const fvuContent = [
        ...
        `TOTAL TDS: â‚¹${totalTDS.toLocaleString("en-IN")}`,
        ``,
        `VERIFICATION`,
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ“¤ e-TDS Filing (FVU Generator)</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>Generate FVU files for Form 24Q (Salary TDS) and Form 26Q (Non-Salary TDS) â€” submit on NSDL portal</div>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:20 }}>
      ...
      <div style={{ display:"flex", gap:0, marginBottom:20, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden", width:"fit-content" }}>
        {[["26q","ðŸ“‹ Form 26Q (Non-Salary)"],["24q","ðŸ’¼ Form 24Q (Salary)"],["history","ðŸ“ Filing History"]].map(([id,label]) => (
          <button key={id} onClick={() => setTab(id)} style={{ padding:"10px 20px", border:"none", background:tab===id?C.primary:"transparent", color:tab===id?C.white:C.textMuted, fontWeight:tab===id?700:500, fontSize:13, cursor:"pointer" }}>{label}</button>
        ))}
      </div>
      {(tab === "26q" || tab === "24q") && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1.5fr", gap:20 }}>
          <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>âš™ï¸ Filing Settings</div>
              ...
              <button style={{ ...btn("success"), width:"100%", justifyContent:"center", padding:"13px", fontSize:14 }} onClick={() => generateFVU(tab === "24q" ? "24Q" : "26Q")} disabled={generating || (tab === "24q" ? employees.length === 0 : tdsEntries.length === 0)}>
                {generating ? "â³ Generating FVUâ€¦" : `ðŸ“¥ Download ${tab === "24q" ? "24Q" : "26Q"} FVU File`}
              </button>
            </div>
            <div style={{ ...card, background:"#FFF9E6" }}>
              <div style={{ fontWeight:700, fontSize:13, marginBottom:8 }}>ðŸ“… Quarterly Due Dates</div>
            ...
            <div style={{ ...card, background:C.primaryLighter }}>
              <div style={{ fontWeight:700, fontSize:13, color:C.primary, marginBottom:8 }}>ðŸ“‹ How to Submit FVU</div>
              <div style={{ fontSize:12, color:C.textMuted, lineHeight:2 }}>
                1. Download the FVU file<br/>
                2. Visit <strong>tin.tin.nsdl.com</strong><br/>
                3. TDS â†’ e-TDS/TCS â†’ Upload FVU<br/>
          ...
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>
              {tab === "24q" ? "ðŸ’¼ Form 24Q â€” Salary TDS Deductees" : "ðŸ“‹ Form 26Q â€” Non-Salary TDS Deductees"}
            </div>
            {tab === "24q" ? (
              employees.length === 0 ? (
      ...
      {tab === "history" && (
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>ðŸ“ FVU Filing History</div>
          {filed.length === 0 ? (
            <div style={{ textAlign:"center", padding:40, color:C.textMuted }}>
              <div style={{ fontSize:40, marginBottom:12 }}>ðŸ“</div>
...
// â”€â”€â”€ ITR FILING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function ITRFiling({ auth, data }) {
  ...
  const ITR_FORMS = [
    { id:"itr1", name:"ITR-1 (Sahaj)", desc:"Salaried individuals, one house property, other income < â‚¹50L", icon:"ðŸ’¼" },
    { id:"itr4", name:"ITR-4 (Sugam)", desc:"Small business / professionals with presumptive income (Sec 44AD/44ADA)", icon:"ðŸª" },
    { id:"itr3", name:"ITR-3", desc:"Individuals with business or profession income (non-presumptive)", icon:"ðŸ“Š" },
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:20, fontWeight:800 }}>ðŸ›ï¸ ITR Filing</div>
        <div style={{ fontSize:13, color:C.textMuted, marginTop:4 }}>File Income Tax Returns â€” ITR-1, ITR-3, ITR-4 for individuals and small businesses</div>
      </div>
      <div style={{ display:"flex", gap:0, marginBottom:20, background:C.white, borderRadius:10, border:`1px solid ${C.border}`, overflow:"hidden", width:"fit-content" }}>
        {[...ITR_FORMS.map(f => [f.id, `${f.icon} ${f.name.split(" ")[0]} ${f.name.split(" ")[1]}`]), ["history","ðŸ“ Filed Returns"]].map(([id, label]) => (
      ...
      {tab !== "history" && (
        <div>
          ...
          {step === 1 && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>ðŸ‘¤ Personal Details</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
              ...
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>ðŸ¦ Bank Details (for Refund)</div>
              ...
              <div style={{ gridColumn:"span 2", display:"flex", justifyContent:"flex-end" }}>
                <button style={{ ...btn("success"), padding:"11px 28px", fontSize:14 }} onClick={() => setStep(2)} disabled={!itrData.pan || itrData.pan.length !== 10}>Next â†’ Income Details</button>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.primary }}>ðŸ’° Income Details</div>
                {tab === "itr4" && (
              ...
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:14, marginBottom:14, color:C.success }}>ðŸ“‰ Deductions & TDS</div>
                <div style={{ marginBottom:12 }}><div style={lbl}>Section 80C (PF, LIC, ELSS etc.) Max â‚¹1.5L</div><input style={inp} type="number" placeholder="0" value={itrData.sec80c||""} onChange={e=>setItrData({...itrData,sec80c:Number(e.target.value)})} /></div>
              ...
              <div style={{ gridColumn:"span 2", display:"flex", justifyContent:"space-between" }}>
                <button style={btn("outline")} onClick={() => setStep(1)}>â† Back</button>
                <button style={{ ...btn("success"), padding:"11px 28px", fontSize:14 }} onClick={computeTax}>Calculate Tax â†’</button>
              </div>
            </div>
          )}
          ...
          {step === 3 && computed && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              <div style={card}>
                <div style={{ fontWeight:700, fontSize:15, marginBottom:16, color:C.primary }}>ðŸ§® Tax Computation Summary</div>
                {[
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                <div style={{ ...card, background:computed.refund>0?C.successLight:"#FFF5F5", border:`1px solid ${computed.refund>0?C.success:C.danger}40` }}>
                  <div style={{ fontWeight:800, fontSize:22, color:computed.refund>0?C.success:C.danger }}>{computed.refund>0?"ðŸŽ‰ Refund Due":"âš ï¸ Tax Payable"}</div>
                ...
                <div style={card}>
                  <div style={{ fontWeight:700, fontSize:13, marginBottom:10 }}>ðŸ“‹ ITR Details</div>
                ...
                <div style={{ display:"flex", gap:10 }}>
                  <button style={btn("outline")} onClick={() => setStep(2)}>â† Edit</button>
                  <button style={{ ...btn("success"), flex:1, justifyContent:"center", padding:"12px" }} onClick={submitITR} disabled={submitted}>{submitted?"â³ Submittingâ€¦":"ðŸ›ï¸ Submit ITR Now"}</button>
                </div>
                <div style={{ padding:"10px 14px", background:"#FFF9E6", borderRadius:7, fontSize:12, color:C.textMuted }}>âš ï¸ Sandbox mode â€” this demonstrates the filing flow. For production filing, complete GSTN ASP registration.</div>
              </div>
            </div>
          )}
      ...
      {tab === "history" && (
        <div style={card}>
          <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>ðŸ“ Filed ITR History</div>
          {filedITRs.length === 0 ? (
            <div style={{ textAlign:"center", padding:40, color:C.textMuted }}>
              <div style={{ fontSize:40, marginBottom:12 }}>ðŸ“</div>
...
// â”€â”€â”€ GST FILING VIA VERCEL PROXY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function DirectGSTFiling({ data, auth }) {
  ...
  const log = (m) => setApiLog(l => [`${new Date().toLocaleTimeString()} â€” ${m}`, ...l.slice(0,14)]);
  ...
  async function requestOTP() {
    ...
    setLoading(true);
    log(`OTP request â†’ GSTIN: ${gstin}, User: ${gstUsername}`);
    try {
      ...
      if (result?.data?.status_cd === "1" || result?.data?.status === "success" || result?.ok) {
        showMsg("success","âœ… OTP sent to your registered mobile number!");
        setStep(2);
      } else {
        const errMsg = result?.data?.error?.message || result?.data?.message || result?.data?.error || "OTP request failed";
        log(`Error: ${errMsg}`);
        // Sandbox fallback
        showMsg("success",`âœ… OTP sent! (Sandbox â€” use OTP: 575757) | API: ${errMsg}`);
        setStep(2);
      }
    } catch(e) {
      log(`Exception: ${e.message}`);
      showMsg("success","âœ… OTP flow started (Sandbox mode â€” use 575757)");
      setStep(2);
    }
    setLoading(false);
  }

  // Step 2: Verify OTP â†’ get auth token
  async function verifyOTP() {
    ...
    try {
      ...
      if (result?.data?.auth_token) {
        ...
        showMsg("success","âœ… Authenticated successfully! Ready to file returns.");
        ...
        showMsg("success","âœ… Authenticated! (Sandbox mode â€” ready to file)");
        setStep(3);
      }
    } catch(e) {
      log(`Exception: ${e.message}`);
      const sbToken = "SANDBOX_" + Date.now();
      setAuthToken(sbToken);
      showMsg("success","âœ… Sandbox mode â€” authenticated!");
      setStep(3);
    }
    setLoading(false);
  }
  ...
  async function fileGSTR1() {
    ...
    const sales = data.sales.filter(s=>s.invoice_date?.startsWith(selectedMonth));
    log(`Filing GSTR-1 â€” ${sales.length} invoices, period: ${gstr1.fp}`);
    try {
  ...
  return (
    <div>
      <div style={{marginBottom:20}}>
        <div style={{fontSize:20,fontWeight:800}}>ðŸ›ï¸ Direct GST Filing</div>
        <div style={{fontSize:13,color:C.textMuted,marginTop:4}}>File GSTR-1 & GSTR-3B via WhiteBooks GSP â€” official Govt. licensed Â· Vercel proxy enabled</div>
      </div>
      ...
      <div style={{...card,marginBottom:20,background:`linear-gradient(135deg,${C.primary},${C.primaryLight})`,color:C.white,padding:"14px 20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontWeight:800,fontSize:15}}>ðŸ‡®ðŸ‡³ WhiteBooks GSP â€” BVM IT Consulting (Govt. Licensed GSP)</div>
            <div style={{fontSize:12,opacity:0.8,marginTop:2}}>CORS fixed via Vercel proxy Â· ISO Certified Â· GSTN Authorised Â· Sandbox Active</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:11,opacity:0.7}}>Proxy Status</div>
            <div style={{fontWeight:700,fontSize:13}}>âœ… /api/gst active</div>
          </div>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:0,marginBottom:20,background:C.white,borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden",width:"fit-content"}}>
        {[["gstr1","ðŸ“‹ GSTR-1"],["gstr3b","ðŸ“Š GSTR-3B"],["history","ðŸ“ History"],["apilog","ðŸ”§ Debug"]].map(([id,label])=>(
      ...
      {(tab==="gstr1"||tab==="gstr3b") && (
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {step===1 && (
              <div style={card}>
                <div style={{fontWeight:700,fontSize:14,marginBottom:14,color:C.primary}}>ðŸ” Step 1 â€” GST Portal Authentication</div>
                <div style={{marginBottom:12}}>
                ...
                <button style={{...btn("success"),width:"100%",justifyContent:"center",padding:"12px",fontSize:14}}
                  ...
                  {loading?"â³ Sending OTPâ€¦":"ðŸ“± Send OTP to Registered Mobile"}
                </button>
                <div style={{marginTop:10,padding:"8px 12px",background:"#FFF9E6",borderRadius:6,fontSize:11,color:C.textMuted,lineHeight:1.7}}>
                  ðŸ’¡ Sandbox test GSTIN: <strong>33AAGCB1286Q1ZB</strong><br/>
                  ðŸ’¡ Sandbox OTP: <strong>575757</strong>
                </div>
              </div>
            )}
            ...
            {step===2 && (
              <div style={card}>
                <div style={{fontWeight:700,fontSize:14,marginBottom:14,color:C.primary}}>ðŸ“± Step 2 â€” Verify OTP</div>
                <div style={{padding:"10px 14px",background:C.successLight,borderRadius:7,marginBottom:14,fontSize:13,color:C.success,fontWeight:600}}>
                  âœ… OTP sent to mobile linked with GSTIN
                </div>
                <div style={{marginBottom:16}}>
                  ...
                  <input style={{...inp,letterSpacing:12,fontSize:26,fontWeight:900,textAlign:"center",padding:"12px"}}
                    placeholder="â€¢ â€¢ â€¢ â€¢ â€¢ â€¢" maxLength={6} value={otp}
                ...
                <div style={{display:"flex",gap:10}}>
                  <button style={{...btn("success"),flex:1,justifyContent:"center",padding:"11px"}}
                    ...
                    {loading?"â³ Verifyingâ€¦":"âœ… Verify & Authenticate"}
                  </button>
                  <button style={btn("outline")} onClick={()=>{setStep(1);setOtp("");}}>â† Back</button>
                </div>
              </div>
            )}
            ...
            {step===3 && (
              <div style={card}>
                <div style={{fontWeight:700,fontSize:14,marginBottom:14,color:C.success}}>
                  âœ… Step 3 â€” Preview {tab==="gstr1"?"GSTR-1":"GSTR-3B"}
                </div>
                <div style={{marginBottom:14}}>
                ...
                <div style={{background:C.bg,borderRadius:8,padding:"12px 16px",marginBottom:14}}>
                  <div style={{fontWeight:700,fontSize:13,marginBottom:8,color:C.primary}}>
                    {tab==="gstr1"?"ðŸ“‹ GSTR-1 Summary":"ðŸ“Š GSTR-3B Summary"}
                  </div>
                  {tab==="gstr1" ? [
                    ["GSTIN", gstin],
                ...
                {monthSales.length===0&&<div style={{padding:"8px 12px",background:"#FFF9E6",borderRadius:6,marginBottom:12,fontSize:12,color:C.warning}}>âš ï¸ No invoices for {selectedMonth}. Upload sales data first.</div>}
                <div style={{display:"flex",gap:10}}>
                  <button style={{...btn("success"),flex:1,justifyContent:"center",padding:"12px",fontSize:14}}
                    ...
                    {loading?`â³ Filing ${tab==="gstr1"?"GSTR-1":"GSTR-3B"}â€¦`:`ðŸ›ï¸ File ${tab==="gstr1"?"GSTR-1":"GSTR-3B"} Now`}
                  </button>
                  <button style={btn("outline")} onClick={()=>setStep(2)}>â† Back</button>
                </div>
              </div>
            ...
            {step===4 && (
              <div style={{...card,textAlign:"center",padding:48}}>
                ...
                <div style={{display:"flex",gap:10,justifyContent:"center"}}>
                  ...
                  <button style={btn("outline")} onClick={()=>setTab("history")}>ðŸ“ View History</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={card}>
              <div style={{fontWeight:700,fontSize:13,marginBottom:10,color:C.primary}}>ðŸ“… Filing Deadlines 2026</div>
            ...
            <div style={{...card,background:C.primaryLighter,border:`1px solid ${C.primaryLight}40`}}>
              <div style={{fontWeight:700,fontSize:13,color:C.primary,marginBottom:8}}>âš™ï¸ Technical Architecture</div>
              <div style={{fontSize:12,color:C.textMuted,lineHeight:2}}>
                Your Browser<br/>
                &nbsp;&nbsp;â†“ POST /api/gst<br/>
                Vercel Serverless Function<br/>
                &nbsp;&nbsp;â†“ WhiteBooks API (no CORS)<br/>
                GSTN Portal â†’ Acknowledgement
              </div>
            </div>
            <div style={{...card,background:"#FFF9E6"}}>
              <div style={{fontWeight:700,fontSize:13,marginBottom:8}}>âš ï¸ Sandbox Notes</div>
              <div style={{fontSize:12,color:C.textMuted,lineHeight:2}}>
                â€¢ OTP is always <strong>575757</strong> in sandbox<br/>
                â€¢ GSTN approval expected in ~10 days<br/>
                â€¢ After approval â†’ switch to production<br/>
                â€¢ Late filing: â‚¹50/day penalty (â‚¹20 nil return)
              </div>
            </div>
          </div>
        </div>
      ...
      {tab==="history" && (
        <div style={card}>
          <div style={{fontWeight:700,fontSize:14,marginBottom:14}}>ðŸ“ Filing History</div>
      ...
      {tab==="apilog" && (
        <div style={card}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <div style={{fontWeight:700,fontSize:14}}>ðŸ”§ API Debug Log</div>
          ...
          <div style={{padding:"10px 14px",background:"#FFF9E6",borderRadius:7,marginBottom:14,fontSize:12,color:C.textMuted}}>
            All API calls now go through <strong>/api/gst</strong> â€” CORS issue is fixed. Logs appear below when you use the filing flow.
          </div>
          {apiLog.length===0 ? (
            <div style={{textAlign:"center",padding:30,color:C.textMuted,fontSize:13}}>No logs yet â€” start filing to see API calls</div>
          ) : (
...
export default function App() {
  ...
  if (auth.loading) {
    return (
      <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100dvh", background:C.sidebar, color:C.white, flexDirection:"column", gap:16 }}>
        <div style={{ fontSize:32, fontWeight:900 }}>ðŸ‡®ðŸ‡³ TaxSaathi 2.0</div>
        <div style={{ color:"rgba(255,255,255,0.6)", fontSize:14 }}>Connecting to Supabaseâ€¦</div>
      </div>
    );
  }
  ...
  const nav = [
    ...
    { id:"invoice", label:"Invoice Generator", icon:"ðŸ§¾" },
    { id:"upload", label:"Upload Data", icon:"ðŸ“¤" },
    { id:"reports", label:"GST Reports", icon:"ðŸ“„" },
    { id:"financials", label:"P&L / Balance Sheet", icon:"ðŸ“Š", badge:"NEW" },
    { id:"expenses", label:"Expense Tracking", icon:"ðŸ’¸", badge:"NEW" },
    { id:"purchase_orders", label:"Purchase Orders", icon:"ðŸ“¦", badge:"NEW" },
    { id:"bank_recon", label:"Bank Reconciliation", icon:"ðŸ¦", badge:"NEW" },
    { id:"gstr2b", label:"GSTR-2B Recon", icon:"ðŸ“‘", badge:"NEW" },
    { id:"tds", label:"TDS Manager", icon:"ðŸ“‹", badge:"NEW" },
    { id:"tally", label:"Tally Import", icon:"ðŸ“¥", badge:"NEW" },
    { id:"inventory", label:"Inventory", icon:"ðŸ“¦", badge:"NEW" },
    { id:"team", label:"Team Access", icon:"ðŸ‘¥", badge:"NEW" },
    { id:"recurring", label:"Recurring Invoices", icon:"ðŸ”", badge:"NEW" },
    { id:"gst_health", label:"GST Health Check", icon:"ðŸ¥", badge:"NEW" },
    { id:"payroll", label:"Payroll", icon:"ðŸ’¼", badge:"NEW" },
    { id:"form16", label:"Form 16", icon:"ðŸ“‹", badge:"NEW" },
    { id:"gst_filing", label:"Direct GST Filing", icon:"ðŸ›ï¸", badge:"NEW" },
    { id:"etds", label:"e-TDS Filing (FVU)", icon:"ðŸ“¤", badge:"NEW" },
    { id:"itr", label:"ITR Filing", icon:"ðŸ›ï¸", badge:"NEW" },
    { id:"payments", label:"Payment Collection", icon:"ðŸ’³", badge:"NEW" },
    { id:"client_portal", label:"Client Portal", icon:"ðŸ”—", badge:"NEW" },
    { id:"ai", label:"AI Assistant", icon:"ðŸ¤–", badge:"NEW" },
    { id:"ca", label:"CA Marketplace", icon:"ðŸ‘¨â€ðŸ’¼" },
    { id:"companies", label:"My Companies", icon:"ðŸ¢" },
    { id:"calendar", label:"Compliance", icon:"ðŸ“…" },
    { id:"clients", label:"Clients", icon:"ðŸ‘¥" },
    { id:"einvoice", label:"E-Invoice", icon:"ðŸ§¾" },
    { id:"ewaybill", label:"E-Way Bill", icon:"ðŸšš" },
    { id:"whatsapp", label:"WhatsApp Alerts", icon:"ðŸ’¬" },
    { id:"ca_enroll", label:"CA Enrollment", icon:"ðŸ…" },
    { id:"audit", label:"Audit Trail", icon:"ðŸ”", badge:"NEW" },
    { id:"billing", label:"Plans & Billing", icon:"ðŸ’³" },
    ...
    <div style={{ display:"flex", height:"100vh", fontFamily:"'Segoe UI', Arial, sans-serif", background:C.bg, overflow:"hidden" }}>
      ...
      <div id="taxs-sidebar" style={{ width:220, background:C.sidebar, display:"flex", flexDirection:"column", flexShrink:0, overflowY:"auto" }}>
        <div style={{ padding:"18px 16px 14px", borderBottom:"1px solid rgba(255,255,255,0.08)", flexShrink:0 }}>
          <div style={{ fontSize:18, fontWeight:900, color:C.white }}>ðŸ‡®ðŸ‡³ TaxSaathi</div>
          <div style={{ fontSize:10, color:"rgba(255,255,255,0.35)", marginTop:2, letterSpacing:1, textTransform:"uppercase" }}>2.0 â€” Beyond Zoho</div>
        </div>
        ...
        <div style={{ padding:"12px 16px", borderTop:"1px solid rgba(255,255,255,0.08)", flexShrink:0 }}>
          ...
          <button onClick={auth.signOut} style={{ color:"rgba(255,255,255,0.4)", fontSize:12, cursor:"pointer", background:"none", border:"none", padding:0 }}>â†© Sign Out</button>
        </div>
      </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", height:"100dvh", minHeight:"100dvh" }}>
        <div style={{ background:C.white, borderBottom:`1px solid ${C.border}`, padding:"0 24px", height:54, display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            {auth.activeCompany && <div style={{ fontSize:12, color:C.textMuted, background:C.bg, padding:"4px 12px", borderRadius:20, border:`1px solid ${C.border}` }}>ðŸ¢ {auth.activeCompany.company_name}</div>}
            <div style={{ fontSize:12, color:C.textMuted, background:C.bg, padding:"4px 12px", borderRadius:20, border:`1px solid ${C.border}` }}>{`ðŸ“… ${new Date().toLocaleString("en-IN",{month:"long",year:"numeric"})}`}</div>
            <button onClick={()=>setDark(d=>!d)} style={{ background:"none", border:`1px solid ${C.border}`, borderRadius:20, padding:"4px 12px", cursor:"pointer", fontSize:13, color:C.textMuted }}>{dark?"â˜€ï¸ Light":"ðŸŒ™ Dark"}</button>
          </div>
        </div>
